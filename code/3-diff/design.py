...
def get_design(dataset_name, latent_name, fragments):
    ...
    cell_latent_space = torch.from_numpy(pickle.load((latent_folder / (latent_name + ".pkl")).open("rb")).values).to(
        torch.float
    )

    general_model_parameters = {"fragments": fragments}

    n_regions_step = min(fragments.n_genes, 5000)
...
def get_folds_training(fragments, folds):
    ...
    for fold in folds:
        ...
        minibatches_train_sets = [
            {"tasks": minibatcher.create_minibatches(use_all=True, rg=np.random.RandomState(i), permute_regions=False)}
            for i in range(10)
        ]
