...
# %%
n_gigs = ((n_data * 32) + (n_data * 32)) / 8 / 1024 / 1024 / 1024
n_gigs
...
# %%
n_gigs = ((n_motifs_total * 32) + (n_motifs_total * 32)) / 8 / 1024 / 1024 / 1024
n_gigs
...
if "chromatinhd.loaders.extraction.motifs" in sys.modules:
    del sys.modules["chromatinhd.loaders.extraction.motifs"]
...
for x, y, s, c in zip(
    ...
    ntscores,
    np.array(["A", "C", "G", "T"])[onehot.argmax(1)],
    np.array(sns.color_palette(n_colors=4))[onehot.argmax(1)],
):
    ax_score.text(
        x,
        y,
        ...
        color="white",
        path_effects=[
            mpl.patheffects.Stroke(linewidth=2, foreground=c),
...
for x, y, s, c in zip(
    np.arange(onehot.shape[0]),
    ntscores,
    np.array(["A", "C", "G", "T"])[onehot_rev.argmax(1)],
    np.array(sns.color_palette(n_colors=4))[onehot_rev.argmax(1)],
):
    ax_scorerev.text(
        x,
        y,
...
# %%
data = chromatinhd.loaders.extraction.fragments.extract_fragments(
    cellxgene_oi,
    cellxgene_indptr,
    coordinates,
    genemapping,
...
loader = chromatinhd.loaders.fragmentmotif.MotifcountsMultiple(
...
# %%
loader = chromatinhd.loaders.fragmentmotif.MotifcountsRelative(
    fragments, motifscan, n_cells * n_genes, window, cutwindow, promoter_width=500
)
...
# %%
from chromatinhd.models.promotermotif.v3 import FragmentSineEncoder
...
# %%
from chromatinhd.models.promotermotif.v3 import FragmentWeighter
...
# %%
from chromatinhd.models.promotermotif.v3 import EmbeddingGenePooler
