...
# %%
design_1 = pd.DataFrame.from_records(itertools.chain(
    itertools.product(
        ["pbmc10k", "e18brain"],
        ["leiden_0.1"],
        ["v4_128-64-32_30_rep"],
...
# designs.append(pfa.utils.crossing(dataset_design, method_design, latent_design, peaks_design, motifscan_design))
...
# %%
scores = {}
for _, (dataset_name, latent_name, method_name, peaks_name, motifscan_name) in design[["dataset", "latent", "method", "peaks", "motifscan"]].iterrows():
    ...
    if (scores_dir / "scores.pkl").exists():
        ...
        print(scores_dir)
scores = pd.concat(scores, names = ["dataset", "method", "peaks", "motifscan", *scores_.index.names])
...
# %%
scores["slope"].values[scores["slope"].values < -0] = 1.
...
peaks_info = pd.DataFrame([
    ...
    ["cellranger"],
    ["rolling_500"]
], columns = ["peaks"]).set_index("peaks")
peaks_info["x"] = np.arange(peaks_info.shape[0])

dataset_info = pd.DataFrame({"dataset":design["dataset"].unique()}).set_index("dataset")
dataset_info["color"] = sns.color_palette(n_colors = len(dataset_info))

# %%
from peakfreeatac import quasirandom

# %%
method_info = pd.DataFrame({
    "method":design["method"].unique()
}).set_index("method")
...
def create_labelling(x_info, col):
    ...
    labelling = pd.DataFrame({"right":y.groupby("group")["x"].max() + 0.5, "left":y.groupby("group")["x"].min() - 0.5})
    labelling["center"] = labelling["left"] + (labelling["right"] - labelling["left"])/2
...
for idx, plotdata_ in plotdata.groupby(group_columns):
    ...
    offset = quasirandom.offset(np.log(plotdata_["slope"]), nbins = 100, method = "quasirandom")/2
    ax.scatter(plotdata_["slope"], x + offset, c = dataset_info.loc[plotdata_["dataset"], "color"], s = 2)
    
    plotdata_agg = plotdata_.groupby("dataset")[["slope"]].mean()
    ax.scatter(plotdata_agg["slope"], np.repeat(x, len(plotdata_agg)), c = dataset_info.loc[plotdata_agg.index, "color"], s = 30, marker = "o", lw = 1, ec = "black")
    ...
    txt = ax.text(m, x, "{:.2f}".format(m), ha = "center", color = "white", va = "center", fontweight = "bold")
    txt.set_path_effects([mpl.patheffects.Stroke(linewidth=1, foreground='#333333FF'), mpl.patheffects.Normal()])
...
# %%
scores_oi = scores.query("peaks == 'cellranger'").query("method == 'v4_128-64-32_30_rep'")
...
# %% tags=[]
fig, axes = plt.subplots(1, len(settings), figsize = (len(settings) * 2, 2), squeeze = False)
for ax, setting in zip(axes[0], settings):
    ...
    ax.scatter(plotdata["n_cells"], plotdata["slope"], c = dataset_info.loc[plotdata["dataset"], "color"], s = 5)
    ax.set_yscale("log")
    ...
    txt.set_path_effects([mpl.patheffects.Stroke(linewidth=3, foreground='#FFFFFF88'), mpl.patheffects.Normal()])
    ...
    ax.annotate(f"$r={lm.rvalue:.2f}$\n$p={lm.pvalue:.2f}$", (0.95, 0.95), xycoords = "axes fraction", ha = "right", fontsize = 8, va = "top")
...
scores = {}
scores_mean = {}
for _, (dataset_name, latent_name, method_name, peaks_name, motfiscan_name, known_motif) in design[["dataset", "latent", "method", "peaks", "motifscan", "known_motif"]].iterrows():
    prediction = Prediction(pfa.get_output() / "prediction_likelihood" / dataset_name / promoter_name / latent_name / method_name)
    ...
    if (scores_dir / "scores.pkl").exists():
        ...
        motifs = scores_.index.get_level_values("motif")[scores_.index.get_level_values("motif").str.contains(known_motif)].unique()
        scores[(dataset_name, method_name, peaks_name, motifscan_name)] = scores_.loc[motifs]
        scores_mean[(dataset_name, method_name, peaks_name, motifscan_name)] = scores_.groupby("group").mean().reset_index()
    else:
        print(scores_dir)
scores = pd.concat(scores, names = ["dataset", "method", "peaks", "motifscan", *scores_.index.names])
scores_mean = pd.concat(scores_mean, names = ["dataset", "method", "peaks", "motifscan", *scores_.index.names])
...
for i, (_, row) in enumerate(plotdata.iterrows()):
    ax.plot([0, 1], row[["odds_peak", "odds_region"]].values, color = dataset_info.loc[row["dataset"], "color"], marker = ".", zorder = -i, alpha = 0.8)
txt = ax.plot([0, 1], np.exp(plotdata[["logodds_peak", "logodds_region"]].mean()), color = "#333", marker = "o")
txt[0].set_path_effects([mpl.patheffects.Stroke(linewidth=3, foreground='#FFFFFF88'), mpl.patheffects.Normal()])
...
# %%
sns.heatmap(scores.query("group == 0")[["logodds_peak", "logodds_region"]].T, vmin = -1, vmax = 1, cmap=mpl.cm.PuOr_r)
