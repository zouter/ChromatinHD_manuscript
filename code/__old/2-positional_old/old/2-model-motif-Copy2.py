...
def test_indexing(indptr, indices, data, distance=None):
    ...
    assert np.array_equal(indices, indices_gs), (
        len(indices),
        len(indices_gs),
        indices.dtype,
        indices_gs.dtype,
...
# %%
idx = np.array([(0, 301)] * 100000)
torch.tensor(1.0, device="cuda")  # init cuda
time_indexing(motifscores_fast, idx=idx, n_idx=idx.shape[0], n=10)
        ...
        diff = torch.tensor(
            np.hstack(
                [
                    np.repeat(
                        self.cutindices,
                        self.indptr[(start + 1) : (end + 1)] - self.indptr[start:end],
                    )
                    for start, end in k
                ]
            )
        )
        return (new_indptr, self.indices[idx], self.data[idx], diff)
...
motif_scores = pd.DataFrame(
    {
        "motif_index": motif_indices.cpu().numpy(),
        "locus_score": locus_scores.cpu().numpy(),
        "locus_distance": locus_distance,
    }
)
