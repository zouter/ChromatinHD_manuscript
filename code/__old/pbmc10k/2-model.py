...
fragments = chromatinhd.fragments.Fragments(pathlib.Path("./"))
...
fragment_embedder = FragmentEmbedder(
    n_hidden_dimensions=100, n_embedding_dimensions=n_embedding_dimensions
)
...
class EmbeddingGenePooler(torch.nn.Module):
    ...
    def forward(self, embedding, fragment_cellxgene_ix, cell_n, gene_n):
        if self.debug:
            assert (
                torch.diff(fragment_cellxgene_ix) >= 0
            ).all(), "fragment_cellxgene_ix should be sorted"
        cellxgene_embedding = torch_scatter.segment_sum_coo(
            fragment_embedding, fragment_cellxgene_ix, dim_size=cell_n * gene_n
        )
...
cell_gene_embedding = embedding_gene_pooler(
    fragment_embedding, fragment_cellxgene_ix, cell_n, gene_n
)
...
class EmbeddingToExpression(torch.nn.Module):
    def __init__(
        ...
        self.weight1 = torch.nn.Parameter(
            torch.empty((n_genes, n_embedding_dimensions), requires_grad=True)
        )
        ...
        self.bias1 = torch.nn.Parameter(
            mean_gene_expression.clone().detach().to("cpu"), requires_grad=True
        )
...
class EmbeddingToExpressionBias(EmbeddingToExpression):
    ...
    def forward(self, cell_gene_embedding, gene_ix):
        return (
            torch.ones(
                (cell_gene_embedding.shape[0], 1), device=cell_gene_embedding.device
            )
            * self.bias1[gene_ix]
        )
...
starts = list(
    itertools.product(
        np.arange(fragments.n_cells, step=n_cell_step),
        np.arange(fragments.n_genes, step=n_gene_step),
    )
)
...
params = itertools.chain(
    fragment_embedder.parameters(),
    embedding_gene_pooler.parameters(),
    embedding_to_expression.parameters(),
)
...
for epoch in range(n_steps):
    for split in splits_training:
        ...
        cell_gene_embedding = embedding_gene_pooler(
            fragment_embedding, split.fragment_cellxgene_ix, split.cell_n, split.gene_n
        )
        expression_predicted = embedding_to_expression(
            cell_gene_embedding, split.gene_ix
        )
    ...
    if (epoch % trace_epoch_every) == 0:
        print(
            f"{epoch} {epoch_mse:.4f} Δ{prev_epoch_mse-epoch_mse if prev_epoch_mse is not None else ''}"
        )
...
# %%
np.corrcoef(
    expression_predicted.detach().cpu().numpy().flatten(),
...
def paircor(x, y, dim=0):
    return ((x - x.mean(dim, keepdims=True)) * (y - y.mean(dim, keepdims=True))).mean(
        dim
    ) / (y.std(dim) * x.std(dim))
...
np.nanmean(
    paircor(
        expression_predicted.detach().cpu().numpy(),
        transcriptome_subset.detach().cpu().numpy(),
        0,
    )
)
...
# %%
gene_scores = (
    explode_dataframe(scores, "genescores")
    .groupby(["phase", "gene"])
    .mean()
    .reset_index()
)
...
gene_scores_grouped["mse_diff"] = (
    gene_scores_grouped["mse"] - gene_scores_grouped["mse_dummy"]
)
gene_scores_grouped["mse_diff"].unstack().T.sort_values("test").plot()

# %%
gene_scores_grouped["symbol"] = transcriptome.symbol(
    gene_scores_grouped.index.get_level_values("gene")
).values
...
for split in tqdm.tqdm(splits):
    for window_idx, (window_start, window_end) in enumerate(zip(cuts[:-1], cuts[1:])):
        ...
        fragments_oi = select_window(
            split.fragments_coordinates, window_start, window_end
        )
        ...
        fragment_embedding = fragment_embedder(
        ...
        genescores = pd.DataFrame(
            {
                ...
                "expression_predicted": expression_predicted_mean.detach()
                .cpu()
                .numpy(),
            }
        )
...
# %%
perc_retained_windows = (
    scores_windows.groupby(["window_mid", "phase"])["perc_retained"].mean().unstack()
)
...
mse_dummy_windows = (
    scores_windows.groupby(["window_mid", "phase"])["mse_dummy"].mean().unstack()
)
...
ax_mse.plot(
    mse_dummy_windows.index, mse_dummy_windows["train"], color="blue", alpha=0.1
)
ax_mse.axhline(mse["train"], dashes=(2, 2), color="blue")

ax_mse2 = ax_mse.twinx()

patch_test = ax_mse2.plot(
    mse_windows.index, mse_windows["test"], color="red", label="test"
)
...
# %%
gene_scores_windows = (
    explode_dataframe(scores_windows, "genescores")
    .groupby(["gene", "phase", "window_mid"])
    .mean()
    .reset_index()
)
...
# %%
gene_mse_windows = (
    gene_scores_windows.groupby(["phase", "gene", "window_mid"])["mse"]
    .first()
    .unstack()
)
gene_perc_retained_windows = (
    gene_scores_windows.groupby(["phase", "gene", "window_mid"])["perc_retained"]
    .first()
    .unstack()
)

# %%
gene_mse_dummy_windows = (
    gene_scores_windows.groupby(["phase", "gene", "window_mid"])["mse_dummy"]
    .first()
    .unstack()
)
...
ax.hist(
    gene_mse_windows.loc["test"].idxmax(1),
    bins=cuts,
    histtype="stepfilled",
    alpha=0.8,
    color="red",
...
ax.hist(
    gene_mse_windows_notss.loc["test"].idxmax(1),
    bins=cuts,
    histtype="stepfilled",
    alpha=0.8,
    color="red",
...
# %%
fig, (ax_mse, ax_perc, ax_peak, ax_bw) = plt.subplots(
    4, 1, height_ratios=[1, 0.5, 0.2, 0.2], sharex=True
)
ax_mse2 = ax_mse.twinx()
...
for split in tqdm.tqdm(splits):
    for window_idx, (window_start, window_end) in enumerate(zip(cuts[:-1], cuts[1:])):
        for window_idx, (window_start2, window_end2) in enumerate(
            zip(cuts[:-1], cuts[1:])
        ):
            # take fragments within the window
            fragments_oi1 = select_window(
                split.fragments_coordinates, window_start, window_end
            )
            fragments_oi2 = select_window(
                split.fragments_coordinates, window_start2, window_end2
            )
            ...
            expression_predicted_dummy = transcriptome_subset.mean(
                0, keepdim=True
            ).expand(transcriptome_subset.shape)
            mse_dummy = loss(expression_predicted_dummy, transcriptome_subset)
            ...
            genescores = pd.DataFrame(
                {
                    ...
                    .numpy(),
                    "mse_dummy": (
                        (expression_predicted_dummy - transcriptome_subset) ** 2
                    )
                    .mean(0)
                    .detach()
                    .cpu()
...
# %%
perc_retained_windowpairs = (
    scores_windowpairs.groupby(["window_mid1", "window_mid2", "phase"])["perc_retained"]
    .mean()
    .unstack()
)
...
mse = scores.groupby("phase")["mse"].mean()
mse_windowpairs = (
    scores_windowpairs.groupby(["window_mid1", "window_mid2", "phase"])["mse"]
    .mean()
    .unstack()
)
mse_dummy_windowpairs = (
    scores_windowpairs.groupby(["window_mid1", "window_mid2", "phase"])["mse_dummy"]
    .mean()
    .unstack()
)
...
# %%
gene_scores_windowpairs = (
    explode_dataframe(scores_windowpairs, "genescores")
    .groupby(["gene", "phase", "window_mid1", "window_mid2"])
    .mean()
    .reset_index()
)

# %%
gene_mse_windowpairs = (
    gene_scores_windowpairs.groupby(["phase", "gene", "window_mid1", "window_mid2"])[
        "mse"
    ]
    .first()
    .unstack()
)
gene_perc_retained_windowpairs = (
    gene_scores_windowpairs.groupby(["phase", "gene", "window_mid1", "window_mid2"])[
        "perc_retained"
    ]
    .first()
...
# %%
sns.heatmap(
    gene_mse_windowpairs.loc["test"].loc[gene_id],
    vmin=gene_mse.loc["test"].loc[gene_id],
)
...
for split in tqdm.tqdm(splits):
    for window_idx, (window_start, window_end) in enumerate(windows):
        # take fragments within the window
        fragment_lengths = (
            split.fragments_coordinates[:, 1] - split.fragments_coordinates[:, 0]
        )
        fragments_oi = ~(
...
mse_dummy_lengths = (
    scores_lengths.groupby(["window_start", "phase"])["mse_dummy"].mean().unstack()
)

# %%
perc_retained_lengths = (
    scores_lengths.groupby(["window_start", "phase"])["perc_retained"].mean().unstack()
)
...
ax_perc.plot(
    perc_retained_lengths.index, perc_retained_lengths["test"], color="#33333344"
)
...
ax2.plot(
    perc_retained_lengths.index, -perc_retained_lengths["train"], color="#33333344"
)
