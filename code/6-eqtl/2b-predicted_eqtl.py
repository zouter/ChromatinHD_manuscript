...
# %%
ground_truth_prioritization = np.clip(ground_truth_variantxgene_effect / variantxgene_effect.values[None, :], 0, 1).to(torch.float32)

# %%
variantxgenes_info["tss_distance"] = genes["tss_position"][variantxgene_index.get_level_values("gene")].values - genotype.variants_info["position"].loc[variantxgene_index.get_level_values("variant")].values
...
minibatches_full = prediction_model.loader.create_bins_ordered(transcriptome.var["ix"].values, n_genes_step = 300)
...
# %%
x = model_pred.variant_embedder.forward(data.relative_coordinates, data.local_clusterxvariant_indptr, data.n_variants, data.n_clusters)
...
z = np.abs(model_pred.ground_truth_variantxgene_effect[
...
class GeneLikelihoodHook:
    ...
    def run_individual(self, model, data):
        self.likelihood_checkpoint[data.variantxgene_ixs.cpu().numpy()] += model.get_full_elbo().cpu().detach().numpy().sum(0).sum(0)
...
for weight, input_bias in zip(model_pred.fc_log_predictor.nn[0].weight[0], model_pred.fc_log_predictor.embedding_bias):
    weight = weight.item()
...
variantxgenes_info.loc[variantxgenes_info.index.get_level_values("gene").isin(validation_genes.index), "phase"] = "validation"
...
# %%
plt.scatter(scores_joined.loc[gene_id]["prioritization"].values, scores_joined.loc[gene_id]["fc_log"].values)
...
indices_oi = (
    ...
    .groupby(["variant", "gene"]).first().index
)
...
# %%
sklearn.metrics.average_precision_score(scores_oi["significant"], scores_oi["prioritization"]) / sklearn.metrics.average_precision_score(scores_oi["significant"], np.random.choice(scores_oi["prioritization"], len(scores_oi)))
...
# %%
sklearn.metrics.log_loss(scores_oi["significant"], scores_oi["prioritization"]) / sklearn.metrics.log_loss(scores_oi["significant"], np.random.choice(scores_oi["prioritization"], len(scores_oi)))
...
# %%
np.corrcoef(scores_joined.xs("NK", level = "cluster")["effect"], scores_joined.xs("NK", level = "cluster")["fc_log"])

# %%
import sklearn.metrics

# %%
sklearn.metrics.average_precision_score(scores_joined.query("cluster == 'CD4 T'")["significant"], scores_joined.query("cluster == 'CD4 T'")["prioritization"])
...
# %%
fig, ax = plt.subplots()
ax.scatter(variantxgenes_info["cor"], paircor(prioritization_oi.values.T, ground_truth_significant))
