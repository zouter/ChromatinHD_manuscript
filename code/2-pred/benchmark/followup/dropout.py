...
dataset_folder = chd.get_output().parent.parent / "chromatinhd" / "output" / "datasets" / "pbmc10k"

# %%
fragments = chd.data.fragments.Fragments(dataset_folder / "fragments" / "10k10k_subset2")
regions = fragments.regions

transcriptome = chd.data.Transcriptome(dataset_folder / "transcriptome_subset2")
...
models = []
for fold in folds:
    model = chd.models.pred.model.additive.Model(fragments = fragments, transcriptome = transcriptome, layer = "magic")
    model.train_model(fold = fold, pbar = True, n_epochs = 60)
    models.append(model)
...
models = []
for fold in folds:
    model = chd.models.pred.model.additive.Model(fragments = fragments, transcriptome = transcriptome, layer = "magic", dropout_rate = 0.1)
    model.train_model(fold = fold, pbar = True, n_epochs = 60)
    models.append(model)
performance_dropout = chd.models.pred.interpret.Performance()
performance_dropout.score(fragments, transcriptome, models, folds)

# %%
models = []
for fold in folds:
    model = chd.models.pred.model.additive.Model(fragments = fragments, transcriptome = transcriptome, layer = "magic", dropout_rate = 0.5)
    model.train_model(fold = fold, pbar = True, n_epochs = 100)
    models.append(model)
performance_dropout = chd.models.pred.interpret.Performance()
performance_dropout.score(fragments, transcriptome, models, folds)

# %%
performance.genescores["cor"].mean("model").mean("gene").plot()
performance_dropout.genescores["cor"].mean("model").mean("gene").plot()
