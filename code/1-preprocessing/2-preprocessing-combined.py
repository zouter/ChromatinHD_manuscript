...
first_dataset_name = "pbmc10k"; second_dataset_name = "pbmc3k"; organism = "hs"
first_dataset_name = "pbmc10k"; second_dataset_name = "lymphoma"; organism = "hs"
first_dataset_name = "lymphoma"; second_dataset_name = "pbmc10k"; organism = "hs"
...
# %%
joined_obs = fragments_first.obs.rename(columns = lambda x: x + "_first").join(fragments.obs.query("original_dataset == @first_dataset_name").reset_index().set_index("cell_original"))
...
new_train_cell_ids = fragments.obs.loc[transcriptome.obs.index[transcriptome.obs["original_dataset"] != first_dataset_name]]["ix"].values.tolist()
genes_all = np.arange(fragments.n_genes)

folds = []
for fold_first in folds_first:
    cells_train = np.array(cell_ix_first_to_cell_ix[fold_first["cells_train"].tolist()].values.tolist() + new_train_cell_ids)
