...
(
    np.trapz(probs, plotdata.loc[ct].index) * 2 / 100 / 100,
    fragments.counts[cells_oi, gene_ix].mean(),
)
...
# %%
# slices = regionpositional.select_windows(gene_id, prob_cutoff = 1.)
slices = regionpositional.select_windows(gene_id, prob_cutoff=-1)
breaking = polyptich.grid.Breaking(slices)

# relative_to = "LSEC_portal"
relative_to = "LSEC_central"
...
# %%
# import eyck
eyck.modalities.transcriptome.plot_umap(transcriptome, [symbol]).display()
...
motifs_oi = motifscan.motifs.loc[
    [
        ...
        # motifscan.motifs.index[motifscan.motifs.index.str.contains("TF7L2")][0],
        # motifscan.motifs.index[motifscan.motifs.index.str.contains("LEF1")][0],
        # motifscan.motifs.index[motifscan.motifs.index.str.contains("TCF7")][0],
        # motifscan.motifs.index[motifscan.motifs.index.str.contains("TF7L1")][0],

        # motifscan.motifs.index[motifscan.motifs.index.str.contains("ZBT20")][0],
        # motifscan.motifs.index[motifscan.motifs.index.str.contains("GATA4")][0],
...
motifs_oi["label"] = motifs_oi["HUMAN_gene_symbol"]
panel_motifs = chd.data.motifscan.plot.GroupedMotifsBroken(
    motifscan, gene_id, motifs_oi, breaking
)
...
# %%
slices = regionpositional.calculate_slices(-1., step=25)

# %%
slice_scores = slices.get_slice_scores()
slice_scores_oi = slice_scores.loc[slice_scores["region_ix"] == fragments.var.index.get_loc(gene_id)]
plt.scatter(slice_scores_oi["start"], slice_scores_oi["length"])
...
# %%
slice_scores = differential_slices.get_slice_scores()
slice_scores_oi = slice_scores.loc[slice_scores["region_ix"] == fragments.var.index.get_loc(gene_id)]
plt.scatter(slice_scores_oi["start"], slice_scores_oi["length"])
...
enrichment.loc[(slice(None), motifs_oi.index), :]

# %%
(
    enrichment.loc["LSEC_central"].query("q_value < 0.01")["log_odds"]
    - enrichment.loc["LSEC_portal"]["log_odds"]
).sort_values().dropna()
...
slicefeatures = []
for _, (chrom, start, end) in slices[
    ["chrom", "start_genome", "end_genome"]
].iterrows():
    if start > end:
        start, end = end, start
    seq = fasta.fetch(chrom, start, end)
    slicefeatures.append(
        {
...
for motif in slicecounts_norm.columns:
    ...
    i = (len(x) // 2) - 1
    y = smooth_spline_fit(
        slicefeatures["GC_norm"][:1000],
        slicecounts[motif][:1000] / slicefeatures["length"][:1000],
        x,
    )
...
fig, ax = plt.subplots()
ax.scatter(
    slicefeatures["GC_norm"],
    slicecounts[motif] / slicefeatures["length"],
    color="blue",
    s=1,
)
ax.scatter(
    slicefeatures["GC_norm"],
    slicecounts_corrected[motif] / slicefeatures["length"],
    color="orange",
    s=1,
)
...
# %%
enrichment.loc[
    enrichment.index.get_level_values("motif").isin(
        motifscan.motifs.loc[
            motifscan.motifs["HUMAN_gene_symbol"].str.contains("RBPJ")
        ].index
    )
].sort_values("odds").style.bar()

# %%
enrichment.loc[
    enrichment.index.get_level_values("motif").str.contains("HEY")
].sort_values("odds").style.bar()
...
motifs_oi["gene"] = (
    adata_raw.var.reset_index()
    .set_index("symbol")
    .groupby("symbol")
    .first()
    .reindex(motifs_oi["symbol"])["gene"]
    .values
)
...
# %%
cluster_transcriptome = (
    pd.DataFrame(
        transcriptome.layers["magic"][:],
...
def score_diffexp_enrichment(
    enrichment: pd.DataFrame, diffexp: pd.DataFrame, motifs_oi, fc_cutoff=1.2
):
    """
    ...
    if "gene" not in motifs_oi.columns:
        raise ValueError(
            "motifs_oi must contain a column 'gene' with the gene id of the motif"
        )
    ...
    for cluster in clusters:
        ...
        if len(subscore) > 4:
            odds = (contingency[1, 1] * contingency[0, 0] + 1) / (
                contingency[1, 0] * contingency[0, 1] + 1
            )
            ...
            if (subscore["lfc"].std() == 0) or (subscore["logodds"].std() == 0):
                ...
                spearman = scipy.stats.spearmanr(subscore["lfc"], subscore["logodds"])[
                    0
                ]
            log_avg_odds = np.concatenate(
                [
                    subscore.query("lfc > 0")["logodds"],
                    -subscore.query("lfc < 0")["logodds"],
                ]
            ).mean()
        else:
        ...
        subscores.append(subscore.assign(cluster=cluster).reset_index())
    ...
    if len(scores):
        ...
        scores = pd.DataFrame(columns=["cluster", "odds", "log_odds", "cor"]).set_index(
            "cluster"
        )
...
# %%
fc_cutoffs = np.linspace(1.0, 2, 20)

cluster_comparison = []

for fc_cutoff in fc_cutoffs:
    scores, _ = score_diffexp_enrichment(
        enrichment, diffexp, motifs_oi, fc_cutoff=fc_cutoff
    )
    cluster_comparison.append(
        scores.assign(method="chd").assign(fc_cutoff=fc_cutoff).reset_index()
    )
    scores_peak, _ = score_diffexp_enrichment(
        enrichment_peak, diffexp, motifs_oi, fc_cutoff=fc_cutoff
    )
    cluster_comparison.append(
        scores_peak.assign(method="peak").assign(fc_cutoff=fc_cutoff).reset_index()
    )
...
cluster_comparison = pd.concat(cluster_comparison, ignore_index=True).set_index(
    ["method", "fc_cutoff", "cluster"]
)

# %%
comparison = cluster_comparison.groupby(["method", "fc_cutoff"])[
    ["cor", "spearman", "log_odds", "log_avg_odds"]
].mean()
comparison["odds"] = np.exp(comparison["log_odds"])
comparison["avg_odds"] = np.exp(comparison["log_avg_odds"])
comparison["contingency"] = cluster_comparison.groupby(["method", "fc_cutoff"])[
    "contingency"
].sum()

# %%
overall_contingency = np.stack(comparison["contingency"].values)
comparison["overall_odds"] = (
    overall_contingency[:, 1, 1] * overall_contingency[:, 0, 0] + 1
) / (overall_contingency[:, 1, 0] * overall_contingency[:, 0, 1] + 1)

# %%
fig = polyptich.grid.Figure(polyptich.grid.Grid(padding_height=0.05, padding_width=0.8))
panel, ax = fig.main.add_right(polyptich.grid.Panel((1.5, 1.5)))
...
# %%
subscores_joined = subscores.query("(lfc > log(@fc_cutoff))").join(
    subscores_peak, lsuffix="_chd", rsuffix="_peak"
)
...
# %%
subscores_joined.query("(logodds_chd < 0) & (logodds_peak > 0)").sort_values(
    "lfc_chd", ascending=False
)["logodds_chd"].plot()
subscores_joined.query("(logodds_chd > 0) & (logodds_peak < 0)").sort_values(
    "lfc_chd", ascending=False
)["logodds_peak"].plot()
...
# %%
subscores_joined_oi = (
    subscores_joined.query("(logodds_chd > 0) & (logodds_peak < 0)")
    .sort_values("lfc_chd", ascending=False)
    .iloc[:10]
)
...
sc.pl.umap(
    ...
    title=[
        "cluster",
        *(
            subscores_joined_oi.index.get_level_values(0)
            + ":"
            + transcriptome.symbol(subscores_joined_oi.index.get_level_values(1))
        ),
    ],
)
...
# %%
fig = polyptich.grid.Figure(polyptich.grid.Grid(padding_width=0.2, padding_height=0.0))
import textwrap

main = fig.main.add_under(polyptich.grid.Grid(padding_width=0.2, margin_height=0.0))
for cluster in clustering.cluster_info.index:
    panel, ax = main.add_right(polyptich.grid.Panel((0.5, 0.5)))
    ax.text(
        0.5,
        0.0,
        "\n".join(textwrap.wrap(cluster, width=10)),
        ha="center",
        va="bottom",
        fontsize=8,
    )
...
for method in ["chd", "peak"]:
    ...
    main = fig.main.add_under(polyptich.grid.Grid(padding_width=0.2))
    ...
    ax.text(
        0.1,
        0.5,
        "{:.2f}".format(np.exp(scores_oi["log_odds"].mean())),
        ha="left",
        va="center",
        fontsize=10,
    )
...
for ct in tqdm.tqdm(clustering.cluster_info.index):
    ...
    genes_oi = diffexp.columns[
        (diffexp.idxmax() == ct) & (diffexp.max() > np.log(expression_fc_cutoff))
    ]

    slicescores_foreground = slicescores.query("cluster == @ct").query(
        "region in @genes_oi"
    )
    slicescores_background = slicescores.query("cluster != @ct").query(
        "region in @genes_oi"
    )

    slicecounts_oi = slicecounts.loc[slicescores_foreground["slice"], motifs_oi]

    slicescores_foreground["slice_ix"] = slicecounts_oi.index.get_indexer(
        slicescores_foreground["slice"]
    )

    print(f"{len(motifs_oi)=} {len(genes_oi)=} {ct=}")

    for gene_oi in genes_oi:
        slicescores_foreground2 = slicescores_foreground.loc[
            (slicescores_foreground["region"] == gene_oi)
        ]
        ...
        founds.append(
            {
                ...
                "ct": ct,
                "motifs": motifs_oi[
                    slicecounts_oi.values[slicescores_foreground2["slice_ix"]].any(0)
                ].tolist(),
            }
        )
...
for ct in tqdm.tqdm(clustering.cluster_info.index):
    ...
    slicescores_peak_foreground = slicescores_peak.query("cluster == @ct").query(
        "region in @genes_oi"
    )
    slicescores_peak_background = slicescores_peak.query("cluster != @ct").query(
        "region in @genes_oi"
    )

    slicecounts_peak_oi = slicecounts_peak.loc[
        slicescores_peak_foreground["slice"], motifs_oi
    ]
        ...
        founds_peak.append(
            {
                ...
                "ct": ct,
                "motifs": motifs_oi[
                    slicecounts_peak_oi.values[
                        slicescores_peak_foreground2["slice_ix"]
                    ].any(0)
                ].tolist(),
            }
        )
...
# %%
founds.join(founds_peak, rsuffix="_peak").query(
    "detected & ~detected_peak"
...
scores = scores.loc[
    clustering.cluster_info.sort_values("n_cells", ascending=False).index
]

fig, ax = plt.subplots()
scores.plot.bar(ax=ax)
ax.set_title(
    "% of genes with at least one differentially accessible binding site", rotation=0
)
