...
df_annotation = pd.DataFrame({"leiden": list(annotation.keys()), "celltype": list(annotation.values())}).set_index(
    "celltype"
)
df_annotation = (
    df_annotation["leiden"].str.split(",").explode().to_frame().reset_index().set_index("leiden")["celltype"]
)
