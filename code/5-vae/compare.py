...
for dataset_name in dataset_names:
    for method_name in method_names:
        prediction = Prediction(pfa.get_output() / "prediction_vae" / dataset_name / promoter_name / method_name)
        if (prediction.path / "scoring" / "overall" / "scores.pkl").exists():
            scores_ = pd.read_pickle(prediction.path / "scoring" / "overall" / "scores.pkl")
...
# %% tags=[]
scores_all = scores.groupby(["dataset", "method", "phase"])[["overlap", "woverlap", "ari", "ami"]].mean()

meanscores = scores.groupby(["dataset","method", "phase"])[["overlap", "woverlap", "ari", "ami"]].mean()
...
# %%
phase_info = pd.DataFrame({"phase":["train", "validation", "all"], "marker":[".", ".", "o"]}).set_index("phase")

# %% tags=[]
fig, axes = plt.subplots(len(metrics), len(dataset_names), figsize = (len(dataset_names) *len(metrics) / 2, 2 * len(method_names) / 2), sharey = True, gridspec_kw = {"wspace":0.05, "hspace":0.2}, squeeze=False)
