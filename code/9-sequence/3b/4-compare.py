...
for run_name, design_row in prediction_design.iterrows():
    prediction = Prediction(pfa.get_output() / "prediction_sequence_v5" / design_row["dataset_name"] / promoter_name / design_row["model_name"])
...
scores_oi = scores.xs(
    ...
    # "test", 
level = "phase")
scores_oi["rmse_relative"].plot(kind = "barh")

# %%
scores_oi = scores.xs("validation", level = "phase")[["rmse_relative"]]
scores_oi.style.bar()

# %%
scores_oi = scores.xs("validation_cell", level = "phase")[["rmse_relative"]]
scores_oi.style.bar()
...
# %%
plt.scatter(
    genescores.loc["v4_nn"].loc["validation"].sort_values("rmse_relative")["rmse_relative"],
    genescores.loc["v4_nn"].loc["validation"].sort_values("rmse_relative")["cor_relative"]
)
...
# %%
fig, ax = plt.subplots()
plt.scatter(genescores.loc["v4"].loc["validation"]["rmse_relative"], genescores.loc["v4_nn"].loc["validation"]["rmse_relative"])
