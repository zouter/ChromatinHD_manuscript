...
#%%
adata_rna = sc.read_loom(
    folder_data_preproc / "GSM6403410_3423-MV-2_gex_possorted_bam_ICXFB.loom"
)

#%%
adata_atac = sc.read_10x_mtx(
    folder_data_preproc / "MV2", var_names="gene_symbols", cache=True, gex_only=False
)
...
adata_rna = adata_rna[
    ...
    & (adata_rna.obs["n_genes_by_counts"] > 1000),
    :,
]
...
df_annotation = pd.DataFrame(
    {"leiden": list(annotation.keys()), "celltype": list(annotation.values())}
).set_index("celltype")
df_annotation = (
    df_annotation["leiden"]
    .str.split(",")
    .explode()
    .to_frame()
    .reset_index()
...
"""
#%%
adata_rna2.obs_names.to_frame().to_csv(
    folder_data_preproc / "filtered_cells.txt", header=False, index=False
)
#%%
...
#%%
adata_result_lin = adata_result[
    adata_result.obs["celltype"].isin(lin_myeloid),
]
...
X2 = np.clip(
    (test - np.quantile(test, 0.01, keepdims=True))
    / (np.quantile(test, 0.99, keepdims=True) - np.quantile(test, 0.01, keepdims=True)),
    0,
    1,
)
...
# %%
fragments_tabix = tabix.open(
    str(folder_data_preproc / "GSM6403411_3423-MV-2_atac_fragments.tsv.gz")
)
...
chromosome_gene_counts = (
    transcriptome.var.groupby("Chromosome").size().sort_values(ascending=False)
)
...
for i in range(n_folds):
    ...
    set1 = set(
        transcriptome.var.index[transcriptome.var["Chromosome"].isin(chromosomes_train)]
    )
    ...
    set1 = set(
        transcriptome.var.index[
            transcriptome.var["Chromosome"].isin(chromosomes_validation)
        ]
    )
    set2 = set(fragments.var.index)
    genes_index = set1.intersection(set2)
...
sizes = []
with gzip.GzipFile(
    folder_data_preproc / "GSM6403411_3423-MV-2_atac_fragments.tsv.gz", "r"
) as fragment_file:
    i = 0
