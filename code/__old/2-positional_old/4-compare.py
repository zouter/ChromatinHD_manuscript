...
for run_name, design_row in prediction_design.iterrows():
    prediction = Prediction(pfa.get_output() / "prediction_positional" / design_row["dataset_name"] / promoter_name / design_row["model_name"])
...
# %%
scores = scores.join((scores - scores.loc["v14_dummy"]).rename(columns = lambda x: x+ "_relative"))

# %%
genescores = genescores.join((genescores - genescores.loc["v14_dummy"]).rename(columns = lambda x: x+ "_relative"))
...
# %%
scores_oi = scores.xs("validation", level = "phase")[["cor"]]
scores_oi.style.bar()

# %%
selfatt_genescores = ((genescores.loc["v15"]["cor"] - genescores.loc["v15_noselfatt"]["cor"])).loc["validation"].sort_values().to_frame()
selfatt_genescores["label"] = transcriptome.symbol(selfatt_genescores.index)

# %%
selfatt_genescores#.query("label == 'PLEKHD1'")

# %%
scores_oi = scores.xs("validation", level = "phase")[["cor_relative"]]
scores_oi.style.bar()
...
# %%
genescores_oi = genescores.loc["v14"].loc["validation"].sort_values("cor_relative", ascending = False)
genescores_oi
