...
for _ in range(200):
    gene_parameters = []
    num_lists = random.randint(2, 10)  # Random number of lists within the range 2-10

    # Generate 'num_lists' lists
    for _ in range(num_lists):
        n_values = random.randint(50, 2000)
        shift_y_density = random.uniform(-2, 2)
        noise = random.uniform(0, 100)
        shift_slope = random.uniform(0, 10)
        p = random.randint(-10000, 10000)
        fragment_length = random.randint(50, 200)
...
pickle.dump(gene_parameters_list_combi, open('gene_parameters_list.pickle', 'wb'))
