...
original_dataset_name = "pbmc10k"; dataset_name = "pbmc10k_leiden_0.1"; organism = "hs"
...
# %%
chromosomes = pd.read_table(folder_data_preproc / "genome" / "chromosome.sizes", names = ["chr", "size"]).set_index("chr").loc[chromosome_names]
...
if not fragments_file.exists():
    (folder_data_preproc / "fragments.tsv.gz").symlink_to(original_folder_data_preproc / "atac_fragments.tsv.gz")
    (folder_data_preproc / "fragments.tsv.gz.tbi").symlink_to(original_folder_data_preproc / "atac_fragments.tsv.gz.tbi")
...
for chromosome, chromosome_info in tqdm.tqdm(chromosomes.iterrows(), total = len(chromosomes)):
...
coordinates = torch.tensor(np.array(coordinates_raw, dtype = np.int64)) # int64 is absolutely needed here to keep within int32_max
coordinates = coordinates.flatten()

chunkcoords = torch.div(coordinates, chunk_size).to(torch.int32) # int32 can be added here since the chunks are much smaller
...
# %%
cluster_to_cells = {}
for cluster, cells in latent.idxmax(1).to_frame("cluster").reset_index().groupby("cluster")["cell"]:
...
target_files = [target_output / file.relative_to(source_output) for file in source_files]
