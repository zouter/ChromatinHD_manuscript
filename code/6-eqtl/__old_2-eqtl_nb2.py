...
source_files = [
    source_output / file.relative_to(target_output) for file in target_files
]
...
expression.values[np.isnan(expression.values)] = 0.0

# make sure the order of the samples is the same in expression and the vcf
# this is not checked afterwards
expression = expression.sel(
    donor=donors_info.reset_index().set_index("old").loc[vcf.samples]["donor"].values
)
...
gene_oi = (
    genes.loc[
        np.abs((genes.query("chr == @gene_oi.chr")["tss"] - gene_oi["tss"]))
        .sort_values()
        .index
    ]
    .query("mean > 0.1")
...
genotype = xr.DataArray(
    genotype - 1, coords=[donors_info.index, pd.Index(variant_ids, name="variant")]
)
# genotype = genotype.sel(variant = ["chr2:203830251:C:T"])
genotype = genotype.sel(
    variant=genotype.coords["variant"]
    .to_pandas()[
        variants_info.loc[genotype.coords["variant"]]["rsid"].isin(qtl_mapped["rsid"])
    ]
    .index
)
...
fc_log_mu = xr.DataArray(
    model.fc_log_mu.detach().cpu().numpy(),
    coords=[cluster_info.index, variants_info_oi.index],
)
...
variant_id, cluster_id = (
    scores.query("cluster == 'NK'").sort_values("lr", ascending=False).index[0]
)
...
sns.heatmap(
    pd.DataFrame(
        np.corrcoef(scores["fc_log_mu"].unstack().T),
        index=cluster_info.index,
        columns=cluster_info.index,
    )
)
...
plotdata = pd.DataFrame(
    {
        # "expression":(expression_oi).to_pandas().unstack()
        "expression": np.log1p((expression_oi / lib) * 10**6)
        .to_pandas()
        .unstack()
    }
)
plotdata["genotype"] = (
    genotype.sel(variant=variant_id)
    .to_pandas()
    .loc[plotdata.index.get_level_values("donor")]
    .values
)
plotdata = plotdata.reset_index()
...
chunks_from, chunks_to = (
    fragments.chunkcoords_indptr[window_chunks[0]],
    fragments.chunkcoords_indptr[window_chunks[1]],
)
...
for cluster_ix, cluster in enumerate(cluster_info.index):
    ...
    ax.scatter(
        plotdata_variants_query["pos"],
        plotdata_variants_query["lr"],
        s=1,
        color="green",
    )
...
fig, ax = plt.subplots()
ld = pd.DataFrame(
    np.abs(np.corrcoef(genotype.T)),
    genotype.coords["variant"].to_pandas(),
    genotype.coords["variant"].to_pandas(),
)
...
contingency = pd.crosstab(
    variants_info_oi["significant_any"],
    variants_info_oi["gwas"],
)
...
# %%
odds = (contingency.values[0, 0] * contingency.values[1, 1]) / (
    contingency.values[0, 1] * contingency.values[1, 0]
)
odds
