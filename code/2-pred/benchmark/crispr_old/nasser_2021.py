...
scoring_folder = model_folder / "scoring" / "crispri" / "nasser_2021"
...
joined_all = joined_all.loc[~pd.isnull(joined_all["HS_LS_logratio"])]
...
# %%
fig, ax =plt.subplots(figsize = (5, 2))
ax.scatter(joined_all["deltacor"], joined_all["HS_LS_logratio"], c = joined_all["significant_expression"], cmap = "viridis", s = 10, vmin = 0, vmax = 1)
...
gene_oi = transcriptome.gene_id("CD83")
...
ax.bar(joined["window_mid"], joined["HS_LS_logratio"], lw = 0, width = binwidth, color = ["tomato" if significant > 0 else "lightgrey" for significant in joined["Significant"]])
...
for gene_oi in tqdm.tqdm(genes_oi):
    for w in ws:
        ...
        if len(joined_oi["significant_expression"].unique()) == 1:
            auroc = 0.5
        else:
        ...
        genescores[method_name].append({
            ...
            "aupr":aupr,
            "auroc":auroc,
            "cor":np.corrcoef(joined_oi["HS_LS_logratio"].abs(), joined_oi["score"] + np.random.normal(0, 1e-6, len(joined_oi)))[0, 1],
            "confirmed":confirmed,
            "w":w,
        })
...
for gene_oi in tqdm.tqdm(genes_oi):
    ...
    for i in range(100):
        ...
        cor.append(np.corrcoef(joined_oi["HS_LS_logratio"].abs(), np.random.rand(joined_oi.shape[0]))[0, 1])
    aupr = np.mean(aupr)
    auroc = np.mean(auroc)
    cor = np.mean(cor)
...
allslicescores_oi = allslicescores_stacked.loc[(allslicescores_stacked["w"] == 5) | (allslicescores_stacked["method"] == "all")]
...
# %%
fig, (ax0, ax1) = plt.subplots(1, 2, figsize = (5, 2))
genescores_oi = genescores_stacked.loc[genescores_stacked["gene"] == transcriptome.gene_id("BLK")]
for method_name, plotdata in genescores_oi.groupby("method"):
    plotdata = plotdata.groupby("w").mean(numeric_only = True).reset_index()
    ax0.plot(plotdata["w"], plotdata["aupr"], label = method_name)
    ax1.plot(plotdata["w"], plotdata["auroc"], label = method_name)
...
plotdata = genescores_stacked.loc[(genescores_stacked["w"] == 5) | (genescores_stacked["method"] == "all")].groupby("method").mean(numeric_only = True).reset_index()
