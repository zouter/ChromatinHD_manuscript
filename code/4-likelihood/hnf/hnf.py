...
cut_local_reflatentxgene_ix = torch.from_numpy(design["reflatent"].values * n_genes + design["gene_ix"].values)
...
transform.unnormalized_widths.data[:, :] = 0.
transform.unnormalized_heights.data[:, :] = 0.
torch.manual_seed(5)
transform.unnormalized_heights.data.normal_(std = 0.5)
transform.unnormalized_heights.data[0, :nbins[0]] = 1.
...
mixture_delta_reflatentxgene[:] = 0.
...
# %%
sns.heatmap((transform.unnormalized_heights.data.cpu().numpy() + mixture_delta_reflatentxgene[0].data.cpu().numpy()))

# %%
logabsdets, outputs = transform.transform_progressive(cut_positions, cut_local_reflatentxgene_ix, cut_local_gene_ix, cut_local_reflatent_ix, mixture_delta_reflatentxgene)

# %%
np.trapz(np.exp(logabsdets[-1].detach().cpu().numpy()), design["position"])

# %%
fig, axes = plt.subplots(1, len(outputs), figsize = (len(logabsdets) * 2, 2), sharey = True, sharex = True)
for (i, output), ax in zip(enumerate(outputs), axes):
    for reflatent, design_reflatent in design.groupby("reflatent"):
        ax.set_aspect(1)
        design_reflatent["position"] = (design_reflatent["coord"] + design_reflatent["gene_ix"])/n_genes
        ...
        ax.scatter(design_reflatent.loc[design_reflatent["coord"] == 0, "position"], output.detach().cpu().numpy()[design_reflatent.index][design_reflatent["coord"] == 0])
...
# %%
inv_logabsdets, samples = transform.transform_progressive(cut_positions, cut_local_reflatentxgene_ix, cut_local_gene_ix, cut_local_reflatent_ix, mixture_delta_reflatentxgene, inverse = True)

# %%
fig, axes = plt.subplots(n_latent, figsize = (20, 1 * n_latent))
for latent_ix, ax in zip(range(n_latent), axes):
    sns.histplot(samples[-1][design["reflatent"] == latent_ix].cpu().detach().numpy(), bins = 1000, ax = ax)
...
# %%
logabsdets, outputs2 = transform.transform_progressive(output, cut_local_reflatentxgene_ix, cut_local_gene_ix, cut_local_reflatent_ix, mixture_delta_reflatentxgene, inverse = True)
