...
# %%
folder = chd.get_output() / "data" / "crispri" / "martyn_2023"
folder.mkdir(parents=True, exist_ok=True)

# %%
# !wget https://www.biorxiv.org/content/biorxiv/early/2023/12/21/2023.12.20.572268/DC1/embed/media-1.xlsx -O {folder / "media-1.xlsx"}

# %%
table = pd.read_excel(folder / "media-1.xlsx", sheet_name="Supplementary Table 8", skiprows=4)

# %%
# convert percent change to fold change


# %%
data_orig = table.copy()
data_orig["chrom"] = data_orig["VariantID"].str.split(":", expand=True)[0]
data_orig["pos"] = data_orig["VariantID"].str.split(":", expand=True)[1].astype(int)
data_orig["HS_LS_logratio"] = (data_orig["% change to PPIF expression"]/100)
data_orig["start"] = data_orig["pos"]
data_orig["end"] = data_orig["start"] + 1
data_orig["gene"] = "PPIF"

# %%
fig, ax = plt.subplots()
plotdata = data_orig.query("gene == 'PPIF'")
...
genes_oi = transcriptome.gene_id(["PPIF"])
...
# %%
joined_all = crispri.calculate_joined(regionmultiwindow, data, genes_oi, fragments.regions, window_size = 50)
