...
model = pickle.load((chd.get_output() / "prediction_likelihood/pbmc10k/10k10k/leiden_0.1/v9_128-64-32/model_0.pkl").open("rb"))
...
genes_oi = torch.tensor([transcriptome.gene_ix("AAK1")])
local_gene_ix = torch.ones(x.shape, dtype = int) * 0
cut_local_cellxgene_ix = torch.ones(x.shape, dtype = int) * 0

# %%
latent = torch.zeros((len(x), n_latent_dimensions))
latent[:, 1] = 0

# %%
mixture_delta, rho_delta = model.decoder(latent.to(torch.float), genes_oi)
...
# %%
y, logabsdet = model.mixture.transform.transform_forward(x, genes_oi, local_gene_ix, mixture_delta)

# %%
fig, (ax0, ax1) = plt.subplots(1, 2)
ax0.plot(x, y.detach().numpy())
ax1.plot(x, np.exp(logabsdet.detach().numpy()))

# %%
z = torch.linspace(0, 1, len(x))

# %%
x_sampled, logabsdet2 = model.mixture.transform.transform_inverse(z, genes_oi, local_gene_ix, mixture_delta)

# %%
fig, (ax0) = plt.subplots(1, 1, figsize = (15, 5))
ax0.hist(x_sampled.detach().cpu().numpy(), bins = 1000, lw = 0, density = True)
ax0.plot(x, np.exp(logabsdet.detach().numpy()))
;
