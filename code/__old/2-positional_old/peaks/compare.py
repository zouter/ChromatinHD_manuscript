...
scores = scores.join(
    pd.DataFrame(
        {"cor_diff": (scores["cor"] - scores.xs(dummy_method, level="method")["cor"])}
    )
)
...
metric = "mse_diff"
metric_multiplier = -1
metric_limits = (-0.1, 0.1)
metric_label = "$\Delta$ mse"
metric = "cor_diff"
metric_multiplier = 1
...
scores_able = (
    scores.query("gene in @genes_oi").groupby(["method", "phase"])[[metric]].mean()
)
...
score_relative_all["better"] = (
    (
        (scores - scores.xs(dummy_method, level="method"))[metric] * metric_multiplier
        > cutoff
    )
    .groupby(["dataset", "method", "phase"])
    .mean()
)
...
score_relative_all["same"] = (
    ((scores - scores.xs(dummy_method, level="method"))[metric].abs() < cutoff)
    .groupby(["dataset", "method", "phase"])
    .mean()
)
