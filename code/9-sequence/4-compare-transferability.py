...
# %%
prediction_design = pfa.utils.crossing(
    pd.DataFrame({"model_name":[
        "v4_dummy",
        "v4",
        "v4_nn"
    ]}),
    pd.DataFrame({"dataset_name":[
        "pbmc10k",
        "pbmc10k+lymphoma",
        "lymphoma",
        "lymphoma+pbmc10k",
...
prediction_design.index = pd.MultiIndex.from_frame(prediction_design)
...
scores = pd.concat(scores, names = [*prediction_design.index.names, "phase"])
scores = scores.drop(columns = ["cos", "mse"])
genescores = pd.concat(genescores, names = [*prediction_design.index.names, "phase", "gene"])
genescores = genescores.drop(columns = ["cos", "mse"])

# %%
scores

# %%
(scores - scores.loc["v4_dummy"]).style.bar()
...
# %%
scores.xs("validation_full", level = "phase").style.bar()
...
# %%
genescores.loc["v4_dummypbmc10k"].loc["validation"]["mse"] - genescores.loc["v4_dummylymphoma+pbmc10k"].loc["validation"]["mse"]

# %%
scores_oi = scores.xs("validation", level = "phase")[["mse_relative"]]
scores_oi.style.bar()
...
# genes_oi = transcriptome.gene_id(["CD3D", "CD19", "CD14"])
...
# %%
import pickle
prediction = Prediction(pfa.get_output() / "prediction_sequence" / dataset_name / promoter_name / "v4")
...
# %%
motif_linear_scores2 = pd.Series(
