...
def aggregate_splits(df, columns=("mse", "mse_dummy"), splitinfo=None):
    ...
    for col in columns:
        df.obj[col + "_weighted"] = (
            df.obj[col] * splitinfo.loc[df.obj["split"], "weight"].values
        )
            ...
            y[column].assign(
                **y[
                    [
                        col
                        for col in y.index
                        if (col != column) and (col not in y[column].columns)
                    ]
                ]
            )
...
# %%
transcriptome_pd = pd.DataFrame(
    transcriptome.X.dense().cpu().numpy(),
...
def score_fold(
    ...
    if use_all_train_splits:
        splits_oi = [split for split in fold if split.phase == "train"] + [
            split for split in fold if split.phase == "validation"
        ]
    else:
        splits_oi = [fold[0]] + [split for split in fold if split.phase == "validation"]

    splitinfo = pd.DataFrame(
        {"split": split_ix, "n_cells": split.cell_n, "phase": split.phase}
        for split_ix, split in enumerate(splits_oi)
    )
    splitinfo["weight"] = (
        splitinfo["n_cells"]
        / splitinfo.groupby(["phase"])["n_cells"].sum()[splitinfo["phase"]].values
    )

    expression_prediction = pd.DataFrame(
        0.0, index=transcriptome.obs.index, columns=transcriptome.var.index
    )
    ...
    for split_ix, split in enumerate(splits_oi):
        try:
            ...
            perc_retained_gene = torch_scatter.scatter_mean(
                fragments_oi_split.float().to("cpu"),
            ...
            with torch.no_grad():
                expression_predicted = model.forward2(
                    split, coordinates, mapping, fragments_oi=fragments_oi_split
                )

        finally:
            split = split.to("cpu")

        transcriptome_subset = transcriptome_X.dense_subset(split.cell_ix)[
            :, split.gene_ix
        ].to(device)

        mse = ((expression_predicted - transcriptome_subset) ** 2).mean()

        expression_predicted_dummy = transcriptome_subset.mean(0, keepdim=True).expand(
            transcriptome_subset.shape
        )
        mse_dummy = ((expression_predicted_dummy - transcriptome_subset) ** 2).mean()

        transcriptome.obs.loc[
            transcriptome.obs.index[split.cell_ix], "phase"
        ] = split.phase

        genescores = pd.DataFrame(
            {
                "mse": ((expression_predicted - transcriptome_subset) ** 2)
                .mean(0)
                .detach()
                .cpu()
                .numpy(),
                "gene": transcriptome.var.index[
                    np.arange(split.gene_ix.start, split.gene_ix.stop)
                ],
                "mse_dummy": ((expression_predicted_dummy - transcriptome_subset) ** 2)
                .mean(0)
                .detach()
                .cpu()
                .numpy(),
        ...
        expression_prediction.values[split.cell_ix, split.gene_ix] = (
            expression_predicted.cpu().detach().numpy()
        )
    ...
    gene_aggscores = (
        scores.pipe(explode_dataframe, "genescores")
        .groupby(["phase", "gene"])
        .pipe(
            aggregate_splits,
            columns=["perc_retained", "mse", "mse_dummy"],
    ...
    cells_train = transcriptome.obs.index[
        list(
            set(
                [
                    cell_ix
                    for split in splits_oi
                    for cell_ix in split.cell_ixs
                    if split.phase == "train"
                ]
            )
        )
    ].unique()
    cells_validation = transcriptome.obs.index[
        list(
            set(
                [
                    cell_ix
    ...
    if expression_prediction_full is not None:
        effect_train = (
            expression_prediction.loc[cells_train].mean()
            - expression_prediction_full.loc[cells_train].mean()
        )
        effect_validation = (
            expression_prediction.loc[cells_validation].mean()
            - expression_prediction_full.loc[cells_validation].mean()
        )
        aggscores["effect"] = pd.Series(
            {"train": effect_train.mean(), "validation": effect_validation.mean()}
        )
        gene_aggscores["effect"] = pd.concat(
            {"train": effect_train, "validation": effect_validation},
            names=["phase", "gene"],
        )

    # calculate correlation
    if calculate_cor:
        cor_train = chd.utils.paircor(
            expression_prediction.loc[cells_train], transcriptome_pd.loc[cells_train]
        )
        cor_validation = chd.utils.paircor(
            expression_prediction.loc[cells_validation],
            transcriptome_pd.loc[cells_validation],
        )

        gene_aggscores["cor"] = pd.concat(
            {"train": cor_train, "validation": cor_validation}, names=["phase", "gene"]
        )
        aggscores["cor"] = pd.Series(
            {"train": cor_train.mean(), "validation": cor_validation.mean()}
        )
...
def score_folds(
    ...
    **kwargs,
    ...
    expression_prediction = pd.DataFrame(
        0.0,
        index=pd.MultiIndex.from_product([fold_ids, transcriptome.obs.index]),
        columns=transcriptome.var.index,
    )

    splitinfo = pd.DataFrame(
        {"split": i, "n_cells": split.cell_n, "phase": split.phase, "fold": fold_ix}
        for fold_ix, fold in enumerate(folds)
        for i, split in enumerate(fold)
    )
    splitinfo["weight"] = (
        splitinfo["n_cells"]
        / splitinfo.groupby(["fold", "phase"])["n_cells"]
        .sum()[pd.MultiIndex.from_frame(splitinfo[["fold", "phase"]])]
        .values
    )
    ...
    for model, (fold_ix, fold) in zip(fold_runs, enumerate(folds)):
        aggscores_, gene_aggscores_, expression_prediction_ = score_fold(
            ...
            expression_prediction_full=expression_prediction_full.loc[fold_ix]
            if expression_prediction_full is not None
            else None,
            **kwargs,
        )
...
# %%
cells_oi = np.random.choice(
    expression_prediction.loc[0].index, size=100, replace=False
)  # only subselect 100 cells
plotdata = pd.DataFrame(
    {"prediction": expression_prediction.loc[0].loc[cells_oi].stack()}
)
...
plotdata = plotdata.loc[
    (plotdata["observed"] > 0) & (plotdata["prediction"] > 0)
]  # only take cells with both values > 0

print(np.corrcoef(plotdata["prediction"], plotdata["observed"])[0, 1])

# %%
plotdata["phase"] = (
    transcriptome.obs["phase"].loc[plotdata.index.get_level_values("cell")].values
)
...
sns.scatterplot(
    x="prediction", y="observed", color="blue", data=plotdata_train, s=1, ax=ax_train
)
correlation = np.corrcoef(plotdata_train["prediction"], plotdata_train["observed"])[
    0, 1
]
ax_train.annotate(
    f"{correlation:.2f}", (0.9, 0.9), xycoords="axes fraction", ha="right", va="top"
)
...
correlation = np.corrcoef(
    plotdata_validation["prediction"], plotdata_validation["observed"]
)[0, 1]
ax_validation.annotate(
    f"{correlation:.2f}", (0.9, 0.9), xycoords="axes fraction", ha="right", va="top"
)
...
# %%
gene_scores["label"] = transcriptome.symbol(
    gene_scores.index.get_level_values("gene")
).values
...
# %%
gene_scores.sort_values("mse_diff", ascending=True).head(20).style.bar(
    subset=["mse_diff", "cor", "mse_dummy"]
)
...
transcriptome.adata.var["log_mse_diff"] = np.clip(
    transcriptome.adata.var["mse_diff"],
    transcriptome.adata.var["mse_diff"].quantile(0.05),
    transcriptome.adata.var["mse_diff"].quantile(0.95),
)
transcriptome.adata.var["log_n_cells"] = np.log10(
    np.array((transcriptome.adata.X > 0).sum(0))[0] + 1
)
transcriptome.adata.var["log_fragments"] = np.log10(
    pd.Series(
        np.bincount(mapping[:, 1].cpu().numpy(), minlength=transcriptome.var.shape[0]),
        fragments.var.index,
    )
    + 1
)
transcriptome.adata.var["log_means"] = np.log10(transcriptome.adata.var["means"] + 1)
transcriptome.adata.var["log_dispersions_norm"] = np.log10(
    transcriptome.adata.var["dispersions_norm"]
)

# %%
# fig, ax = plt.subplots()
sns.pairplot(
    transcriptome.adata.var,
    vars=[
        "log_means",
        "log_n_cells",
        "log_mse_diff",
        "log_fragments",
        "log_dispersions_norm",
    ],
    diag_kind=None,
)
# ax.set_xscale("log")
...
for window_start, window_end in zip(cuts, cuts[1:] + [9999999]):
    windows.append({"window_start": window_start, "window_end": window_end})
...
for window_id, (window_start, window_end) in tqdm.tqdm(
    windows.iterrows(), total=windows.shape[0]
):
    # take fragments within the window
    fragment_lengths = fragments.coordinates[:, 1] - fragments.coordinates[:, 0]
    fragments_oi = ~(
        (fragment_lengths >= window_start) & (fragment_lengths < window_end)
    )

    aggscores_window, gene_aggscores_window, _ = score_folds(
        coordinates,
        mapping,
        folds,
        models,
        fragments_oi,
        expression_prediction_full=expression_prediction,
    )
...
ax_perc.plot(
    perc_retained_lengths.index, perc_retained_lengths["validation"], color="#33333344"
)
...
# %%
fig, ax = plt.subplots()
(mse_lengths.std() * (zscore(mse_lengths) - zscore(1 - perc_retained_lengths)))[
    "validation"
].plot()
...
# %%
gene_scores_lengths["label"] = pd.Categorical(
    transcriptome.symbol(gene_scores_lengths.index.get_level_values("gene")).values
)
gene_scores_lengths["perc_lost"] = 1 - gene_scores_lengths["perc_retained"]

# %%
gene_normmse_lengths = gene_scores_lengths.loc["validation"]["mse"].unstack().pipe(
    zscore, dim=1
) - gene_scores_lengths.loc["validation"]["perc_lost"].unstack().pipe(zscore, dim=1)
...
# %%
gene_order = (
    gene_scores.loc["validation"].sort_values("mse_diff", ascending=False).index
)
...
gene_aggscores_windows["mse_loss"] = (
    gene_aggscores["mse"] - gene_aggscores_windows["mse"]
)
...
patch_train = ax_mse.plot(
    mse_windows.index, mse_windows["train"], color="blue", label="train"
)
...
patch_validation = ax_mse2.plot(
    mse_windows.index, mse_windows["validation"], color="red", label="validation"
)
...
lm.fit(
    1 - scores_windows.loc["validation"][["perc_retained"]],
    scores_windows.loc["validation"]["mse"],
)
mse_residual = scores_windows.loc["validation"]["mse"] - lm.predict(
    1 - scores_windows.loc["validation"][["perc_retained"]]
)
...
ax.plot(
    ...
    zscore(scores_windows.loc["validation"]["mse"])
    - zscore(1 - scores_windows.loc["validation"]["perc_retained"]),
    color="red",
)
...
fig, ax = plt.subplots()
ax.hist(
    gene_mse_windows.loc["train"].idxmax(1),
    bins=cuts,
    histtype="stepfilled",
    alpha=0.5,
    color="blue",
...
ax.hist(
    gene_mse_windows.loc["validation"].idxmax(1),
    bins=cuts,
    histtype="stepfilled",
    alpha=0.8,
    color="red",
...
# %%
gene_mse_windows_notss = gene_mse_windows.loc[
    :, (cuts[:-1] < -1000) | (cuts[:-1] > 1000)
]

# %%
fig, ax = plt.subplots()
ax.hist(
    gene_mse_windows_notss.loc["train"].idxmax(1),
    bins=cuts,
    histtype="stepfilled",
    alpha=0.5,
    color="blue",
...
ax.hist(
    gene_mse_windows_notss.loc["validation"].idxmax(1),
    bins=cuts,
    histtype="stepfilled",
    alpha=0.8,
    color="red",
...
# %%
gene_mse_windows_norm = (
    gene_mse_windows - gene_mse_windows.values.min(1, keepdims=True)
) / (
    gene_mse_windows.values.max(1, keepdims=True)
    - gene_mse_windows.values.min(1, keepdims=True)
)

# %%
sns.heatmap(
    gene_mse_windows_norm.loc["validation"].loc[
        gene_scores.loc["validation"].sort_values("mse_diff", ascending=False).index
    ]
)
...
peaks_bed = pybedtools.BedTool(
    chd.get_output() / "peaks" / dataset_name / "macs2" / "peaks.bed"
)
...
# %%
fig, (ax_mse, ax_effect, ax_perc, ax_peak, ax_bw) = plt.subplots(
    5, 1, height_ratios=[1, 0.5, 0.5, 0.2, 0.2], sharex=True
)
ax_mse2 = ax_mse.twinx()
...
ax_mse2.axhline(
    gene_scores.loc[("validation", gene_id), "mse"], dashes=(2, 2), color="red"
)

# mse
plotdata = gene_mse_windows.loc["train"].loc[gene_id]
patch_train = ax_mse.plot(
    plotdata.index, plotdata, color="grey", label="train", alpha=0.3
)
...
promoter_updown["type"] = (
    pd.Series(["nothing", "down"])[
        promoter_updown["down"].values.astype(int)
    ].reset_index(drop=True)
    + "_"
    + pd.Series(["nothing", "up"])[
        promoter_updown["up"].values.astype(int)
    ].reset_index(drop=True)
).values
...
sns.scatterplot(
    x=promoter_updown["max_increase"],
    y=promoter_updown["max_decrease"],
...
promoter_updown.query("type == 'nothing_up'").sort_values(
    "max_increase", ascending=False
)
...
# %%
sns.scatterplot(
    x=gene_scores_windows["mse_loss"], y=gene_scores_windows["perc_retained"], s=1
)

# %%
gene_mse_correlations = gene_scores_windows.groupby(["phase", "gene"]).apply(
    lambda x: x["perc_retained"].corr(x["mse"])
)
...
gene_mse_correlations["label"] = transcriptome.symbol(
    gene_mse_correlations.index.get_level_values("gene")
).values
...
gene_mse_correlations.loc["validation"].query("cor > =-.5").sort_values(
    "mse_diff", ascending=False
)
...
special_genes["n_fragments_importance_not_correlated"] = (
    gene_mse_correlations.loc["validation"]["cor"] > -0.5
)
...
# %%
sns.scatterplot(
    x=gene_scores_windows["effect"], y=gene_scores_windows["perc_retained"], s=1
)

# %%
perc_retained_cutoff = gene_scores_windows.loc["validation"]["perc_retained"].quantile(
    0.05
)

# %%
gene_scores_windows.query("perc_retained < @perc_retained_cutoff").loc[
    "validation"
].index.get_level_values("gene").unique()

# %%
gene_effect_correlations = (
    gene_scores_windows.query("mse_diff > @mse_loss_cutoff")
    .groupby(["phase", "gene"])
    .apply(lambda x: x["perc_retained"].corr(x["effect"]))
)
# gene_mse_correlations = gene_aggscores_windows.query("perc_retained < 0.98").groupby(["phase", "gene"]).apply(lambda x: x["perc_retained"].corr(x["mse"]))
gene_effect_correlations = gene_effect_correlations.to_frame("cor")
gene_effect_correlations = gene_effect_correlations[
    ~pd.isnull(gene_effect_correlations["cor"])
]
gene_effect_correlations["label"] = transcriptome.symbol(
    gene_effect_correlations.index.get_level_values("gene")
).values
gene_effect_correlations["mse_diff"] = gene_scores["mse_diff"]

# %%
genes_oi = (
    gene_scores.loc["validation"]
    .query("mse_diff > 1e-3")
    .index.intersection(gene_effect_correlations.index.get_level_values("gene"))
)
print(f"{len(genes_oi)=}")
...
gene_effect_correlations.loc["validation"].query("cor <= .5").sort_values(
    "mse_diff", ascending=False
)
...
# %%
gene_scores_tss = gene_scores_windows.xs(
    key=windows.iloc[-100].name, level="window"
).copy()
gene_scores_tss["label"] = transcriptome.symbol(
    gene_scores_tss.index.get_level_values("gene")
).values
...
assert (
    select_cutwindow(np.array([[-100, 200], [-300, -100]]), -50, 50)
    == np.array([True, True])
).all()
assert (
    select_cutwindow(np.array([[-100, 200], [-300, -100]]), -310, -309)
    == np.array([True, True])
).all()
assert (
    select_cutwindow(np.array([[-100, 200], [-300, -100]]), 201, 202)
    == np.array([True, True])
).all()
assert (
    select_cutwindow(np.array([[-100, 200], [-300, -100]]), -200, 20)
    == np.array([False, False])
).all()
...
for window_id, (window_start, window_end) in tqdm.tqdm(
    ...
    aggscores_cutwindow, gene_aggscores_cutwindow, _ = score_folds(
        coordinates,
        folds,
        models,
        fragments_oi,
...
gene_scores_cutwindows = gene_aggscores_cutwindows.groupby(
    ["phase", "gene", "window"]
).mean()
...
lm = sklearn.linear_model.LinearRegression().fit(
    plotdata_frag[["perc_retained"]], plotdata_frag["mse_loss"]
)
...
# %%
lm = sklearn.linear_model.LinearRegression().fit(
    scores_cutwindows[["perc_retained"]], scores_cutwindows["mse_loss"]
)
cut_residual = scores_cutwindows["mse_loss"] - lm.predict(
    scores_cutwindows[["perc_retained"]]
)
...
for _, (window_start1, window_end1, window_start2, window_end2) in tqdm.tqdm(
    windowpairs[
        ["window_start1", "window_end1", "window_start2", "window_end2"]
    ].iterrows(),
    total=windowpairs.shape[0],
):
    # take fragments within the window
...
aggscores_windowpairs = aggscores_windowpairs.set_index(
    ["window1", "window2"], append=True
)

gene_aggscores_windowpairs = pd.concat(gene_aggscores_windowpairs)
gene_aggscores_windowpairs = gene_aggscores_windowpairs.set_index(
    ["window1", "window2"], append=True
)

aggscores_windowpairs["mse_loss"] = aggscores["mse"] - aggscores_windowpairs["mse"]
gene_aggscores_windowpairs["mse_loss"] = (
    gene_aggscores["mse"] - gene_aggscores_windowpairs["mse"]
)

scores_windowpairs = aggscores_windowpairs.groupby(
    ["phase", "window1", "window2"]
).mean()
gene_scores_windowpairs = gene_aggscores_windowpairs.groupby(
    ["phase", "gene", "window1", "window2"]
).mean()
...
reference_idx = scores_windowpairs.index.get_level_values(
    "window2"
) == scores_windowpairs.index.get_level_values("window1")
...
# %%
scores_windowpairs_test = scores_windowpairs.join(reference1[cols], rsuffix="1").join(
    reference2[cols], rsuffix="2"
)
...
scores_windowpairs_test["perc_loss12"] = (
    scores_windowpairs_test["perc_loss1"] + scores_windowpairs_test["perc_loss2"]
)
scores_windowpairs_test["perc_loss_bias"] = (
    scores_windowpairs_test["perc_loss"] / scores_windowpairs_test["perc_loss12"]
)
...
for col in cols:
    scores_windowpairs_test[col + "_interaction"] = (
        scores_windowpairs_test[col] - scores_windowpairs_test[f"{col}12"]
    )
...
# %%
plotdata = scores_windowpairs_test.loc[("validation")][
    "perc_loss_interaction"
].unstack()
np.fill_diagonal(plotdata.values, 0)
sns.heatmap(plotdata, cmap=mpl.cm.RdBu_r, center=0.0, vmin=-1e-5, vmax=1e-5)
...
np.fill_diagonal(plotdata.values, 0)
sns.heatmap(plotdata, cmap=mpl.cm.RdBu, center=0.0)
...
# %%
gene_scores_windowpairs["mse_loss"] = (
    gene_scores["mse"] - gene_scores_windowpairs["mse"]
)
...
reference_idx = gene_scores_windowpairs.index.get_level_values(
    "window2"
) == gene_scores_windowpairs.index.get_level_values("window1")
...
# %%
gene_scores_windowpairs_test = gene_scores_windowpairs.join(
    reference1[cols], rsuffix="1"
).join(reference2[cols], rsuffix="2")

# %%
# calculate the bias
gene_scores_windowpairs_test["perc_loss12"] = (
    gene_scores_windowpairs_test["perc_loss1"]
    + gene_scores_windowpairs_test["perc_loss2"]
)
eps = 1e-8
...
for col in cols:
    gene_scores_windowpairs_test[col + "12"] = (
        gene_scores_windowpairs_test[col + "1"]
        + gene_scores_windowpairs_test[col + "2"]
    ) * (gene_scores_windowpairs_test["perc_loss_bias"])

# calculate the interaction
for col in cols:
    gene_scores_windowpairs_test[col + "_interaction"] = (
        gene_scores_windowpairs_test[col] - gene_scores_windowpairs_test[f"{col}12"]
    )
...
# %%
col = "effect"
plotdata_all = gene_scores_windowpairs_test.loc[
    ("validation", gene_scores_windowpairs_test.index.get_level_values("gene")[0])
]
...
# %%
gene_scores_windowpairs_test["label"] = transcriptome.symbol(
    gene_scores_windowpairs_test.index.get_level_values("gene")
).values

# %%
# if you're interested in genes with a strong interaction
gene_scores_windowpairs_test.loc["validation"].sort_values(
    "mse_loss_interaction", ascending=True
).groupby("gene").first().sort_values("mse_loss_interaction", ascending=True).head(10)[
    ["mse_loss_interaction", "label"]
]
...
plt.hist(
    np.bincount(
        fragments.mapping[:, 0][
            (fragments.mapping[:, 1] == fragments.var.loc[gene_id]["ix"])
        ].cpu()
    )
)
