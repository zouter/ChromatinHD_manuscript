...
ax = axes[0]
sns.ecdfplot(
    np.exp(peakaverages["average"]), ax=ax, color=methods_info.loc["peak", "color"]
)
sns.ecdfplot(
    np.exp(regionaverages["average"]), ax=ax, color=methods_info.loc["region", "color"]
)
# sns.ecdfplot(np.exp(pureregionaverages["average"]), ax = ax, color = methods_info.loc["region", "color"], dashes = (2, 2))
...
ax = axes[1]
sns.ecdfplot(
    np.exp(peakresult.get_sliceaverages(probs_interpolated)["max"]),
    ax=ax,
    color=methods_info.loc["peak", "color"],
)
sns.ecdfplot(
    np.exp(regionresult.get_sliceaverages(probs_interpolated)["max"]),
    ax=ax,
    color=methods_info.loc["region", "color"],
)
# sns.ecdfplot(np.exp(pureregionresult.get_sliceaverages(probs_interpolated)["max"]), ax = ax, color = methods_info.loc["region", "color"], dashes = (2, 2))
ax.set_xscale("log")
