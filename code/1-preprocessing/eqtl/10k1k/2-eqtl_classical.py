...
for _, gene_oi in tqdm.tqdm(genes_oi.iterrows(), total = len(genes_oi)):
    ...
    slope, p = chd.qtl.mapping.calculate_spearman_univariate_multiinput_multioutput(X, Y)
    ...
    data["variant_ix"].append(np.tile(variants_info.loc[variant_ids]["ix"].values, len(cluster_info)))
    data["gene_ix"].append(np.repeat(gene_oi["ix"], len(cluster_info) * len(variant_ids)))
...
for cluster in expression.columns:
    ...
    expression_clusters[cluster] = [
        expression_cluster.values,
        samples_profiled
    ]
...
for cluster, (y_all, samples_profiled) in expression_clusters.items():
    gt = pd.Categorical(np.array(["ref/ref", "alt/ref", "-", "alt/alt"])[variant.gt_types], categories = ["ref/ref", "alt/ref", "alt/alt"])
