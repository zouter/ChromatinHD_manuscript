...
for dataset_name in [
    ...
    for prediction_name, design_row in design.items():
        ...
        loaders_all = pfa.loaders.LoaderPool(
            design_row["loader_cls"],
            design_row["loader_parameters"],
            n_workers = 5,
            shuffle_on_iter = False
        ...
        for fold_ix, fold in [(fold_ix, fold) for fold_ix, fold in enumerate(folds)][fold_slice]:
            ...
            lr = 1e-3
            optimizer = pfa.optim.SparseDenseAdam(model.parameters_sparse(), model.parameters_dense(), lr = lr, weight_decay = 1e-5)
            n_epochs = 200
            ...
            for ax, plotdata_mixture, plotdata_counts in [
                ...
                [axes[2], likelihood_mixture.mean()[-50:], likelihood_counts.mean()[-50:]]
            ]:
