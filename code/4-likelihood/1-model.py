...
# %%
coordinates, mapping, cell_latent_space = (
    torch.from_numpy(simulation.coordinates).to(torch.float32),
    torch.from_numpy(simulation.mapping),
    torch.from_numpy(simulation.cell_latent_space).to(torch.float32),
)
...
for dim in range(cell_latent_space.shape[1]):
    fragments_oi = (cell_latent_space[:, dim] > 0)[cellmapping] & (
        genemapping == gene_ix
    )
    ax.hist(
        coordinates.cpu().numpy()[fragments_oi, 0],
        bins=1000,
        range=window,
        histtype="step",
    )
...
order = np.argsort(
    (simulation.mapping[:, 0] * simulation.n_genes) + simulation.mapping[:, 1]
)
...
fragments_original = chd.data.Fragments(
...
fragments.obs = pd.DataFrame({"cell": np.arange(fragments_original.n_cells)}).set_index(
    "cell"
)
fragments.create_cellxgene_indptr()
...
fragments_oi = torch.isin(
    fragments_original.genemapping, torch.from_numpy(gene_ixs.values)
)
...
fragments.mapping[:, 1] = torch.from_numpy(
    mapper[fragments_original.mapping[fragments_oi][:, 1]]
)
...
model = vae_model.Decoding(
    fragments,
    torch.from_numpy(latent.values),
    nbins=(
        128,
        64,
        32,
