...
# %%
fig = chd.grid.Figure(chd.grid.Wrap(ncol = 2, padding_height = 0.15))
...
for method in methods_oi:
    ...
    plotdata["score"] = plotdata["score"].abs()
    plotdata["HS_LS_logratio2"] = plotdata.groupby("score")["HS_LS_logratio"].transform("mean")
    panel, ax = fig.main.add(chd.grid.Panel((1.3, 1.3)))
    cors = []
    auprs = []
    aurocs = []
    for gene, plotdata in plotdata.groupby("gene"):
...
# CRISPRI
panel, ax = fig.main.add_under(chd.grid.Panel((width, 0.55)))
ax.bar(
    joined["window_mid"],
    joined["HS_LS_logratio"],
    lw=0,
