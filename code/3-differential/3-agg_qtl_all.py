...
def aggregate_motifscores(motifscores):
    ...
    ratio_up = ((motifscores_oi.query("n_region > 0")["in_region"]) > (motifscores_oi.query("n_region > 0")["in_peak"])).mean() / ((motifscores_oi.query("n_region > 0")["in_region"]) < (motifscores_oi.query("n_region > 0")["in_peak"])).mean()
    ratio_detected = ((motifscores_oi.query("n_region > 0")["in_region"] > 0).sum() / (motifscores_oi.query("n_region > 0")["in_peak"] > 0).sum())
...
# %%
((motifscores_oi["in_region"])-(motifscores_oi["in_peak"])).groupby("gene").min().mean()

# %%
((motifscores_oi["in_region"])-(motifscores_oi["in_peak"])).groupby("gene").max().mean()

# %%
((motifscores_oi.query("n_region > 0")["in_region"]) == (motifscores_oi.query("n_region > 0")["in_peak"])).mean()
...
methods_info["label"] = methods_info.index.get_level_values("diffexp") + " " + methods_info.index.get_level_values("peakcaller")
...
metrics_info = pd.DataFrame([
    ...
    {"label":r"ratio up", "metric":"logratio_up", "limits":(np.log(1/2), np.log(2)), "ticks":[np.log(1/2), 0, np.log(2)], "ticklabels":["½", "1", "2"]},
    {"label":r"ratio detected", "metric":"logratio_detected", "limits":(np.log(1/2), np.log(2)), "ticks":[np.log(1/2), 0, np.log(2)], "ticklabels":["½", "1", "2"]},
]).set_index("metric")
...
# %%
np.exp(meanscores.groupby(["dataset", "motifscan", "diffexp", "peakcaller"]).mean()[metrics_info.index]).style.bar()
