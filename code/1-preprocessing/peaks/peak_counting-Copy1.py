...
peakcounts.count_peaks(
    ...
    transcriptome.adata.obs["ix"].astype(str).values,
)
...
classifier.fit(
    adata_atac.obsm["X_pca"][cells_train],
    adata_atac.obs["leiden_transcriptome"].astype(int)[cells_train],
)
prediction = classifier.predict(adata_atac.obsm["X_pca"][cells_validation])
sklearn.metrics.balanced_accuracy_score(
    adata_atac.obs["leiden_transcriptome"].astype(int)[cells_validation], prediction
)
...
def sk_indices_to_sparse(indices):
    ...
    A = scipy.sparse.coo_matrix(
        (
            np.repeat(1, k * indices.shape[0]),
            (indices[:, 1:].flatten(), np.repeat(np.arange(indices.shape[0]), k)),
        ),
...
# %%
import chromatinhd.loaders.peakcounts

# %%
loader = chd.loaders.peakcounts.Peakcounts(fragments, peakcounts)
...
# %%
model = pickle.load(
    (chd.get_output() / "prediction_vae/pbmc10k/10k10k/stack/pca_50/model_0.pkl").open(
        "rb"
    )
)
...
adata_atac = sc.AnnData(
    peakcounts.counts.astype(np.float32), obs=transcriptome.obs, var=peakcounts.var
)
sc.pp.normalize_total(adata_atac)
sc.pp.log1p(adata_atac)
sc.pp.highly_variable_genes(adata_atac)
...
# %%
peakscores = (
    sc.get.rank_genes_groups_df(adata_atac, group=group_id)
    .rename(columns={"names": "peak", "scores": "score"})
    .set_index("peak")
)
peakscores_joined = peakcounts.peaks.join(peakscores, on="peak").sort_values(
...
motif_indices = []
n = 0
for _, (relative_start, relative_end, gene_ix) in (
    peakscores_joined[["relative_start", "relative_end", "gene_ix"]]
    .loc[positive]
    .iterrows()
):
...
# %%
def count_motifs(
    relative_starts, relative_end, gene_ixs, motifscan_indices, motifscan_indptr
):
    motif_indices = []
    n = 0
    for relative_start, relative_end, gene_ix in tqdm.tqdm(
        zip(relative_starts, relative_end, gene_ixs)
    ):
        start_ix = gene_ix * (window[1] - window[0]) + relative_start - window[0]
        end_ix = gene_ix * (window[1] - window[0]) + relative_end - window[0]
        motif_indices.append(
            motifscan_indices[motifscan_indptr[start_ix] : motifscan_indptr[end_ix]]
        )
        n += relative_end - relative_start
    motif_indices = np.hstack(motif_indices)
...
# %%
motif_counts, n = count_motifs(
    peakscores_joined.loc[positive, "relative_start"],
    peakscores_joined.loc[positive, "relative_end"],
    peakscores_joined.loc[positive, "gene_ix"],
    motifscan.indices,
    motifscan.indptr,
)

# %%
motif_counts2, n2 = count_motifs(
    peakscores_joined.loc[negative, "relative_start"],
    peakscores_joined.loc[negative, "relative_end"],
    peakscores_joined.loc[negative, "gene_ix"],
    motifscan.indices,
    motifscan.indptr,
)
...
# %%
motif_counts2, n2 = np.bincount(
    motifscan.indices, minlength=motifs_oi.shape[0]
), fragments.n_genes * (window[1] - window[0])

# %%
motifscores = pd.DataFrame(
    {"odds": (motif_counts / n) / (motif_counts2 / n2), "motif": motifs_oi.index}
).set_index("motif")
...
def count_gc(relative_starts, relative_end, gene_ixs, onehot_promoters):
    ...
    for relative_start, relative_end, gene_ix in tqdm.tqdm(
        ...
        gc.append(
            onehot_promoters[start_ix:end_ix, [1, 2]].sum() / (end_ix - start_ix + 1e-5)
        )
...
# %%
promoter_gc = count_gc(
    torch.ones(fragments.n_genes, dtype=torch.int) * window[0],
    torch.ones(fragments.n_genes, dtype=torch.int) * window[1],
    torch.arange(fragments.n_genes),
    onehot_promoters,
)
sns.histplot(promoter_gc, bins=np.linspace(0, 1, 50), stat="density")
plt.axvline(promoter_gc.mean(), color="blue")

window_oi_gc = count_gc(
    position_slices[:, 0], position_slices[:, 1], gene_ixs_slices, onehot_promoters
)
sns.histplot(window_oi_gc, bins=np.linspace(0, 1, 50), stat="density")
...
random_position_slices[:, 0] = np.random.randint(
...
random_gene_ixs_slices = np.random.randint(
    fragments.n_genes, size=random_position_slices.shape[0]
)
...
random_difference = np.abs(
    (
        window_random_gc.reshape((position_slices.shape[0], n_random))
        - window_oi_gc[:, None]
    )
)
...
sns.histplot(window_random_gc, bins=np.linspace(0, 1, 50), stat="density")
...
sns.histplot(window_background_gc, bins=np.linspace(0, 1, 50), stat="density")
...
# %%
motifscores2 = pd.read_csv(chd.get_output() / "a.csv", index_col=0)
...
linreg = scipy.stats.linregress(
    motifscores2.loc[motifs_oi, "logodds"], motifscores.loc[motifs_oi, "logodds"]
)
slope = linreg.slope
