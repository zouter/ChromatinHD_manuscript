...
minibatch = chromatinhd.loaders.minibatches.Minibatch(cells_oi=cells_oi, genes_oi=genes_oi)
...
loaders = chromatinhd.loaders.pool.LoaderPoolOld(
    ...
    {"fragments": fragments, "cellxregion_batch_size": n_cells * n_genes},
    n_workers=2,
)
...
for i in range(2):
    ...
    data.append(chd.loaders.minibatching.Minibatch(cells_oi=cells_oi, genes_oi=genes_oi))
...
prediction = chd.flow.Flow(chd.get_output() / "prediction_positional" / dataset_name / promoter_name / prediction_name)
