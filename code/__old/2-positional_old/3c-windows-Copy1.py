...
# %%
# scores_dir = (prediction.path / "scoring" / ("windows" + "_500"))
...
genescores_overall["label"] = transcriptome.symbol(genescores_overall.index.get_level_values("gene")).values
...
def join_overall(df, columns = (overallmetric, "label")):
    ...
    return df.drop(columns = todrop).join(genescores_overall.loc["validation", columns]).sort_values(overallmetric, ascending = False)
...
# %%
scores.loc["validation"]["retained"].plot(color = phases.loc["validation", "color"])
...
patch_train = ax_mse2.plot(mse_windows.index, effect_windows["train"], color = phases.loc["train", "color"], label = "train")

patch_validation = ax_mse.plot(mse_windows.index, effect_windows["validation"], color = phases.loc["validation", "color"], label = "validation", zorder = 10)

ax_mse2.set_ylabel("effect train", rotation = 0, ha = "left", color = phases.loc["train", "color"])
ax_mse.set_ylabel("effect validation", rotation = 0, ha = "right", color = phases.loc["validation", "color"])
...
mse_residual = (1-scores.loc["validation"][metric] - lm.predict(scores.loc["validation"][["retained"]])) / scores.loc["validation"][metric].std()

# %%
fig, ax = plt.subplots(figsize = (3, 2))
patch_train = ax.plot(mse_residual.index, mse_residual, color = phases.loc["validation", "color"], label = "validation")
...
fig, ax = plt.subplots(figsize = (3, 2))
patch_train = ax.plot(mse_residual.index[window_oi], mse_residual[window_oi], color = phases.loc["validation", "color"], label = "validation")
...
mse_residual = (1-scores.loc["validation"]["effect"] - lm.predict(scores.loc["validation"][["retained"]])) / scores.loc["validation"]["effect"].std()
...
ax.plot(
    ...
    zscore(scores.loc["validation"][metric]) - zscore(1-scores.loc["validation"]["retained"]),
...
# %%
genescores["label"] = transcriptome.symbol(genescores.index.get_level_values("gene")).values

# %%
fig, ax = plt.subplots()
ax.hist(genecor.loc["validation"].idxmax(1), bins = genecor.columns, histtype = "stepfilled", alpha = 0.8, color = phases.loc["validation", "color"])
...
# %%
fig, ax = plt.subplots()
ax.hist(genecor_notss.loc["validation"].idxmax(1), bins = genecor.columns, histtype = "stepfilled", alpha = 0.8, color = phases.loc["validation", "color"])
fig.suptitle("Most important window across genes outside of TSS")

# %%
genecor_norm = (genecor - genecor.values.min(1, keepdims = True)) / (genecor.values.max(1, keepdims = True) - genecor.values.min(1, keepdims = True))
...
fig = pfa.grid.Figure(peakfreeatac.grid.Wrap(ncol = 1, padding_height=0.001))

w = 5

ax_title_ = fig.main.add(pfa.grid.Title(transcriptome.symbol(gene_id) + " promoter", dim = (w, 0.8)))

limits = (design.index[0], design.index[-1])
# limits = (-1000, 1000)

# correlation
ax_cor_ = fig.main.add(pfa.grid.Ax(dim = (5, 1)))
ax_cor = ax_cor_.ax
ax_cor2 = ax_cor.twinx()

# cor annot
ax_cor2.set_ylabel("$\\Delta$ cor\ntrain", rotation = 0, ha = "left", va = "center",color = phases.loc["train", "color"])
ax_cor.set_ylabel("$\\Delta$ cor\nvalidation", rotation = 0, ha = "right", va = "center", color = phases.loc["validation", "color"])
...
patch_train = ax_cor2.plot(plotdata.index, plotdata, color = phases.loc["train", "color"], label = "train", alpha = 0.3)
plotdata = genescores.loc["validation"].loc[gene_id][metric]
patch_validation = ax_cor.plot(plotdata.index, plotdata, color = phases.loc["validation", "color"], label = "train")

# effect
ax_effect_ = fig.main.add(pfa.grid.Ax(dim = (5, 1)))
ax_effect = ax_effect_.ax
plotdata = geneeffect.loc["train"].loc[gene_id]
patch_train = ax_effect.plot(plotdata.index, plotdata, color = phases.loc["train", "color"], label = "train")
plotdata = geneeffect.loc["validation"].loc[gene_id]
patch_validation = ax_effect.plot(plotdata.index, plotdata, color = phases.loc["validation", "color"], label = "train")
...
# fragnents
ax_fragments_ = fig.main.add(pfa.grid.Ax(dim = (5, 1)))
...
ax_fragments.plot(plotdata.index, plotdata, color = phases.loc["validation", "color"], label = "train")
...
# peaks
ax_peak_ = fig.main.add(pfa.grid.Ax(dim = (5, 0.15 * peak_methods.shape[0])))
ax_peak = ax_peak_.ax
for _, peak in peaks.iterrows():
...
# bw
ax_bw_ = fig.main.add(pfa.grid.Ax(dim = (5, 0.5)))
ax_bw = ax_bw_.ax
ax_bw.plot(
...
promoter_updown = pd.DataFrame({
    "best_enhancer":-geneeffect.loc["validation"].min(1),
    "best_repressor":+geneeffect.loc["validation"].max(1)
})
...
promoter_updown["type"] = (
    pd.Series(["nothing", "repressor"])[promoter_updown["repressor"].values.astype(int)].reset_index(drop = True) + 
    "_" + 
    pd.Series(["nothing", "enhancer"])[promoter_updown["enhancer"].values.astype(int)].reset_index(drop = True)
).values
...
sns.scatterplot(x = promoter_updown.loc[genes_oi]["best_enhancer"], y = promoter_updown.loc[genes_oi]["best_repressor"], hue = promoter_updown["type"])
...
promoter_updown.loc[genes_oi].query("repressor").sort_values("cor_diff", ascending = False).head(10)
...
# %%
# gene_mse_correlations = genescores.groupby(["phase", "gene"]).apply(lambda x: x["retained"].corr(-x["cor"]))
gene_mse_correlations = genescores.query("retained < @retained_cutoff").groupby(["phase", "gene"]).apply(lambda x: x["retained"].corr(x[metric]))
gene_mse_correlations = gene_mse_correlations.to_frame("retained_cor")
gene_mse_correlations = gene_mse_correlations[~pd.isnull(gene_mse_correlations["retained_cor"])]

# %%
overall_cutoff = 0.01

# %%
genes_oi = genescores_overall.loc["validation"].index[genescores_overall.loc["validation"][overallmetric] > overall_cutoff]
...
# %%
genes_oi2 = genescores_overall.loc["validation"].index[genescores_overall.loc["validation"]["cor"] > cutoff]
print(f"{len(genes_oi2)=}")

# %%
fig, ax = plt.subplots(figsize = (5, 4))
sns.ecdfplot(gene_mse_correlations.loc["validation"].loc[:, "retained_cor"], ax = ax, label = "all")
sns.ecdfplot(gene_mse_correlations.loc["validation"].loc[genes_oi, "retained_cor"], ax = ax, label = f"{overallmetric} > {overall_cutoff} ({len(genes_oi)})")
sns.ecdfplot(gene_mse_correlations.loc["validation"].loc[genes_oi2, "retained_cor"], ax = ax, label = f"cor > {cutoff} ({len(genes_oi2)})")
ax.set_ylabel("ECDF", rotation = 0, ha = "right", va = "center")
ax.set_xlabel("Correlation between a window's predictive power and # fragments", rotation = 0, ha = "center", va = "top")
...
gene_mse_correlations.loc["validation"].query("retained_cor > .8").pipe(join_overall).head(10)
...
gene_mse_correlations.loc["validation"].query("retained_cor < -0.").pipe(join_overall).head(10)
...
# %%
special_genes["n_fragments_importance_not_correlated"] = gene_mse_correlations.loc["validation"]["retained_cor"] < 0.5
...
# %%
geneeffect_correlations = genescores.query("deltacor_mask").groupby(["phase", "gene"]).apply(lambda x: x["retained"].corr(x["effect"]))
geneeffect_correlations = geneeffect_correlations.to_frame("retained_cor")
geneeffect_correlations = geneeffect_correlations[~pd.isnull(geneeffect_correlations["retained_cor"])]

# %%
cor_diff_cutoff = 0.005

# %%
genes_oi = genescores_overall.loc["validation"].query("cor_diff > @cor_diff_cutoff").index
genes_oi = genes_oi.intersection(geneeffect_correlations.loc["validation"].index.get_level_values("gene"))
print(f"{len(genes_oi)=}")
...
genes_oi2 = genes_oi2.intersection(geneeffect_correlations.loc["validation"].index.get_level_values("gene"))
print(f"{len(genes_oi2)=}")

# %%
fig, ax = plt.subplots(figsize = (5, 4))
sns.ecdfplot(geneeffect_correlations.loc["validation"].loc[:, "retained_cor"], ax = ax, label = "all")
sns.ecdfplot(geneeffect_correlations.loc["validation"].loc[genes_oi, "retained_cor"], ax = ax, label = f"Δcor > 0.01 ({len(genes_oi)})")
sns.ecdfplot(geneeffect_correlations.loc["validation"].loc[genes_oi2, "retained_cor"], ax = ax, label = f"cor > 0.1 ({len(genes_oi2)})")
ax.set_ylabel("ECDF", rotation = 0, ha = "right", va = "center")
ax.set_xlabel("Correlation between a window's effect and # fragments", rotation = 0, ha = "center", va = "top")
ax.legend(title = "genes")
ax.axvline(0, dashes = (2, 2), color = "#333333")
# ax.hist(gene_mse_correlations.loc["validation"].loc[:, "retained_cor"], range = (-1, 1), density = True, histtype = "stepfilled")
# ax.hist(gene_mse_correlations.loc["validation"].loc[genes_oi, "retained_cor"], range = (-1, 1), density = True)
...
ax.hist(geneeffect_correlations.loc["validation"].loc[genes_oi, "retained_cor"], range = (-1, 1))
ax.hist(geneeffect_correlations.loc["validation"].loc[genes_oi2, "retained_cor"], range = (-1, 1))
...
# if you're interested in genes with a low correlation and a good prediction
geneeffect_correlations.loc["validation"].query("retained_cor < 0.2").join(genescores_overall[["cor_diff", "label"]]).sort_values("cor_diff", ascending = False)
...
# %%
genescores_tss = genescores.xs(key = design.loc[-50.].name, level = "window").copy()
...
# %%
sns.ecdfplot(genescores_tss.loc["validation"].query(metric + " < @cutoff").sort_values("effect")["effect"])
...
matched_peaks, matched_windows = np.where(((localpeaks["start"].values[:, None] < np.array(design)[:, 1][None, :]) & (localpeaks["end"].values[:, None] > np.array(design)[:, 0][None, :])))

# %%
peak_window_matches = pd.DataFrame({"peak":localpeaks.index[matched_peaks], "window":design.index[matched_windows], "gene":localpeaks["gene"].iloc[matched_peaks]}).set_index("peak").reset_index()

# %% [markdown]
# ### Is the most predictive window inside a peak?

# %%
gene_best_windows = genescores.loc["validation"].loc[genescores.loc["validation"].groupby(["gene"])[metric].idxmin()]
genes_oi = genescores_overall.loc["validation"].query(overallmetric + " > @overall_cutoff").index
...
ax.annotate(annot, xy=(top_cutoff, 1-perc_within_a_peak), xycoords=("data", "data"), xytext = (0, 10), textcoords = "offset points", va = "bottom", ha = "center", color = "red", bbox=dict(fc="#FFFFFF88"))
...
# %%
genescores_matched = genescores.loc["validation"].join(peak_window_matches.set_index(["gene", "window"])).groupby(["gene", "window"]).first().reset_index(level = "window")
...
genescores_matched["cum_matched"] = (np.cumsum(genescores_matched["matched"]) / genescores_matched["ix"])
...
# %%
top_cutoff = 0.05
perc_within_a_peak = genescores_matched["cum_matched"].iloc[int(genescores_matched.shape[0] * top_cutoff)]
print(perc_within_a_peak)
...
# %%
genescores_matched_oi = genescores_matched.iloc[:int(top_cutoff * genescores_matched.shape[0])]
...
annot = f"{1-perc_within_a_peak:.2%}"
ax.annotate(annot, xy=(top_cutoff, 1-perc_within_a_peak), xycoords=("data", "data"), xytext = (0, 10), textcoords = "offset points", va = "bottom", ha = "right", color = "red", bbox=dict(fc="#FFFFFF88"))
ax.xaxis.set_major_formatter(mpl.ticker.PercentFormatter(xmax = 1))
ax.yaxis.set_major_formatter(mpl.ticker.PercentFormatter(xmax = 1))
ax.set_ylim(0, 1)
...
genescores_matched_summary.pipe(join_overall).query("perc_matched < 0.2").query("n_windows > 10").sort_values("cor_diff", ascending = False)

# %%
genescores_matched.to_pickle(scores_dir / ("genescores_matched_" + peaks_name + ".pkl"))

# %%
special_genes["predictive_positions_not_in_peak"] = genescores_matched.iloc[:int(genescores_matched.shape[0] * top_cutoff)].groupby("gene")["matched"].all()
...
gene_peak_scores = pd.DataFrame({
    "effect_min":genescores_matched.query("matched").groupby(["gene", "peak"])["effect"].min(),
    "effect_max":genescores_matched.query("matched").groupby(["gene", "peak"])["effect"].max(),
    "deltacor_min":genescores_matched.query("matched").groupby(["gene", "peak"])["deltacor"].min(),
    "deltacor_sum":genescores_matched.query("matched").groupby(["gene", "peak"])["deltacor"].sum(),
    "window_mean":genescores_matched.query("matched").groupby(["gene", "peak"])["window"].mean()
})
...
ax.annotate(annot, xy=(top_cutoff, perc_updown), xycoords=("data", "data"), xytext = (0, 10), textcoords = "offset points", va = "bottom", ha = "center", color = "red", bbox=dict(fc="#FFFFFF88"))
ax.set_xlabel("Top genes (acording to cor)")
...
gene_peak_scores.query("updown")[["window_mean", "deltacor_sum", "effect_max", "effect_min"]].pipe(join_overall).sort_values("deltacor_sum", ascending = True).head(15)
...
# %%
matched_scores = genescores_matched.groupby(["gene", "matched"])["deltacor"].sum().unstack()
matched_scores
...
# %%
genescores_matched_loci = genescores_matched.groupby(["gene", "matched"])["deltacor"].sum().unstack()
...
ax.bar(np.arange(len(gene_order)), -genescores_matched_loci.loc[gene_order, True], width = 1, lw = 0)
ax.bar(np.arange(len(gene_order)), -genescores_matched_loci.loc[gene_order, False], bottom = -genescores_matched_loci.loc[gene_order, True], width = 1, lw = 0)
...
# %%
def match_deltacor_retained(df, deltacor_quantile = 0.9, retained_quantile = 0.9):
    return (
...
# %%
peak_max_matches = genescores_matched.query("matched").groupby(["gene", "peak"]).apply(match_deltacor_retained, deltacor_quantile = 0.9, retained_quantile = 0.9)
...
ax.annotate(annot, xy=(top_cutoff, perc_notmatched), xycoords=("data", "data"), xytext = (0, 10), textcoords = "offset points", va = "bottom", ha = "center", color = "red", bbox=dict(fc="#FFFFFF88"), arrowprops={"arrowstyle":"-", "ec":"red"})
ax.set_xlabel("Top genes (acording to cor)")
ax.set_title("% peaks where summit does not match top $\Delta$ cor", rotation = 0, loc='left')

# %%
# if you're interested in genes where one peak's maximum does not match with the most predictive window
peak_max_scores.loc[~peak_max_scores["match"]].pipe(join_overall).sort_values("deltacor_sum").head(10)
...
peak_max_scores["distance"] = (
    genescores_matched.reset_index().set_index("window").groupby(["gene", "peak"])["deltacor"].idxmin() - 
    genescores_matched.reset_index().set_index("window").groupby(["gene", "peak"])["retained"].idxmin()
)
...
# %%
fig, ax = plt.subplots()
sns.ecdfplot(np.abs(peak_max_scores.query("perc < @top_cutoff")["distance"]))
ax.set_ylabel("ECDF", rotation = 0, ha = "right", va = "center")
ax.set_xlabel("Distance between summit and most predictive locus")
...
peak_max_scores = pd.DataFrame({
    "retained_cor":genescores_matched.query("matched").groupby(["gene", "peak"]).apply(lambda x: x["retained"].corr(x[metric])),
    "window_mean":genescores_matched.query("matched").groupby(["gene", "peak"])["window"].mean(),
    "deltacor_sum":genescores_matched.query("matched").groupby(["gene", "peak"])["deltacor"].sum(),
})
peak_max_scores = peak_max_scores.sort_values("deltacor_sum", ascending = True)
...
peak_max_scores["perc"] = peak_max_scores["ix"] / peak_max_scores.shape[0]
peak_max_scores["cum_retained_cor"] = np.cumsum(peak_max_scores["retained_cor"]) / peak_max_scores["ix"]

# %%
top_cutoff = 0.2
retained_cor = peak_max_scores["cum_retained_cor"].iloc[int(peak_max_scores.shape[0] * top_cutoff)]
print(retained_cor)
print(f"Perhaps within a peak there is not really a correlation between peak height and importance?\nIndeed, {retained_cor} is the average correlation for the top {top_cutoff:.0%} peaks.")

# %%
fig, ax = plt.subplots(figsize = (5, 3))
ax.scatter(peak_max_scores["perc"], peak_max_scores["retained_cor"], s = 0.3, color = "#3335")
...
ax.annotate(annot, xy=(top_cutoff, retained_cor), xycoords=("data", "data"), xytext = (0, -10), textcoords = "offset points", va = "top", ha = "center", color = "red", bbox=dict(fc="#FFFFFF88"), arrowprops={"arrowstyle":"-", "ec":"red"})
ax.set_xlabel("Top peaks (acording to $\Delta$ cor)")
ax.set_title("Correlation within peaks of # fragments and $\Delta$ cor", rotation = 0, loc='left')
