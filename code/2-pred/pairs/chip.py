...
step = 100
pad = (5000 // step) * 2
indices = np.arange(0, fragments.regions.window_width, step)
...
# %%
window_to_windowix = regionmultiwindow.design.set_index("window_mid")["ix"]

# %%
windowscores.sort_values("deltacor1")
...
for gene in tqdm.tqdm(genes_oi):
    ...
    if gene in regionpairwindow.interaction:
        interaction = regionpairwindow.interaction[gene].mean("fold").to_dataframe("cor").assign(gene = gene).reset_index()
        ...
        interaction["distance"] = abs(interaction["window1_mid"] - interaction["window2_mid"])

        interaction["deltacor1"] = regionmultiwindow.scores["deltacor"].sel_xr(gene).sel(phase = "test").mean("fold").to_pandas()[interaction["window1"]].values

        interaction["cor"] = np.clip(interaction["cor"], 0, np.inf)
        interaction["right"] = (
            interaction["window2_mid"] > interaction["window1_mid"]
        ) * interaction["cor"] > 0
        interaction["abscor"] = np.clip(interaction["cor"].abs(), 0, 1)

    # if regionmultiwindow.scores["scored"].sel_xr(gene).all():
    #     interaction = regionmultiwindow.scores["deltacor"].sel_xr(gene).sel(phase="test").mean("fold").to_dataframe("deltacor").reset_index()
    #     interaction["window1_mid"] = regionmultiwindow.design.loc[interaction["window"], "window_mid"].values
    #     interaction["deltacor1"] = interaction["deltacor"]
    #     interaction["cor"] = 0.
    #     interaction["abscor"] = 0.
    #     interaction["distance"] = 9999
    #     interaction["right"] = 0.

        windowscores = (
            interaction.query("distance > 1000")
            .sort_values("cor", ascending=False)
            .groupby("window1_mid")[["cor", "deltacor1", "right", "abscor"]]
...
# %%
foci["ix"] = np.arange(len(foci))
accession_oi = files.index[files["target"] == "POLR2A"][0]
...
for accession_oi in tqdm.tqdm(files.index):
    ...
    for gene, foci_gene in tqdm.tqdm(
        (
            foci
            # .query("deltacor1 < -1e-3")
            # .query("cor > 0.001")
            # .query("(window < -1000) | (window > 1000)")
            # .query("window < -1000")
            # .query("window > 1000")
        ...
        cutoff = foci_gene["deltacor1"].median()
        # cutoff = foci_gene["deltacor1"].quantile(0.75)
        foci_oi = foci_gene.loc[
            True
            # & (foci_gene["abscor"] > foci_gene["abscor"].median())
            & (foci_gene["deltacor1"] < cutoff)
            & True
        ]
        ...
        ratio = np.nanmean(pads_oi + 1e-3, 0) / np.nanmean(pads_oi2 + 1e-3, 0)
...
for accession_oi, ratios_accession in ratios.items():
    enrichments[accession_oi] = np.exp(
        np.nanmean(np.log(np.stack(ratios_accession)), 0)*1.5
    )
...
manuscript.save_figure(fig, "5", "loop_enrichment")
# %%
